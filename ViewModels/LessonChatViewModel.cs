...
public partial class LessonChatViewModel : ViewModelBase, IDisposable
{
    ...
    {
        return Enum.TryParse<ConversationModeVoiceProvider>(providerName, ignoreCase: true, out var provider)
            ? provider
            : ConversationModeVoiceProvider.Tts1;
    }
    ...
    private void LogRealtimeRecordState(string reason)
    {
        Debug.WriteLine(
            ...
            $"IsConversationModeActive={IsConversationModeEnabled}; ConversationModeVoiceProvider={CurrentConversationModeVoiceProvider}; IsRealtimeConversationActive={IsRealtimeConversationActive}; " +
    ...
    private void LogTextInputState(string reason, bool canSend)
    {
        Debug.WriteLine(
            ...
            $"SoftWrapUpTurn={GetSoftWrapUpTurn()}; " +
            $"FinalTurn={GetFinalTurn()}; " +
            $"HasWrapUpStarted={hasTriggeredWrapUp}; " +
            $"RuntimeContentSource={lessonScenario.RuntimeContent.Source}; " +
            $"RuntimeContentEffectiveSource={lessonScenario.RuntimeContent.EffectiveSource}; " +
            $"ContentPackSlug={lessonScenario.RuntimeContent.ContentPackSlug}; " +
            $"RuntimeContentVersion={lessonScenario.RuntimeContent.VersionNumber?.ToString(CultureInfo.InvariantCulture) ?? string.Empty}; " +
            $"RuntimeContentHash={lessonScenario.RuntimeContent.SnapshotHash}; " +
    ...
    private async Task StartVoiceRecordingAsync()
    {
        ...
        try
        {
            ...
            if (IsTtsConversationModeActive)
            {
                ...
                SetConversationModeState(ConversationModeState.Recording, "tts_record_start_requested");
            }
            IsRecording = true;
            RefreshAvatarState();
    ...
    private async Task StopVoiceRecordingAsync()
    {
        ...
        try
        {
            ...
            if (IsTtsConversationModeActive)
            {
                SetConversationModeState(ConversationModeState.WaitingForTranscript, "tts_recording_saved");
            }
            ...
            if (!transcriptValidation.IsValid)
            {
                ...
                if (IsTtsConversationModeActive)
                {
                    ConversationLatestUserText = LessonTranscriptValidator.InvalidTranscriptUserMessage;
                    SetConversationModeState(ConversationModeState.Ready, "tts_invalid_user_transcript");
                }
            ...
            if (IsTtsConversationModeActive)
            {
                ConversationLatestUserText = trimmedTranscriptionText;
                HideConversationHint();
            }
        ...
        finally
        {
            ...
            SetIsTranscribingAudio(false);
            IsRecording = false;
            if (IsTtsConversationModeActive && !IsSending && !IsBotVoicePlaying && !IsCompletedAwaitingFinish)
            {
                SetConversationModeState(ConversationModeState.Ready, "tts_recording_stop_finished");
            }
            RefreshAvatarState();
    ...
    private async Task ToggleConversationModeAsync()
    {
        ...
        var operationThresholdMilliseconds = isDisablingConversationMode
            ? UiOperationWarningThresholdMilliseconds
            : (IsRealtimeConversationProviderEnabled ? RealtimeStartupWarningThresholdMilliseconds : UiOperationWarningThresholdMilliseconds);
        var operationStopwatch = StartUiOperationDiagnostics(
            operationName,
            operationThresholdMilliseconds,
            $"Provider={CurrentConversationModeVoiceProvider}; SessionId={realtimeSessionId}; State={CurrentConversationModeState}; CurrentLessonPhase={CurrentLessonPhase}; IsConversationModeEnabled={IsConversationModeEnabled}");
        ...
        try
        {
            ...
            {
                await StopConversationModeAsync("user_exit_conversation_mode");
                LogLessonStateSnapshot("Conversation Mode toggle off");
                return;
            }

            await StartConversationModeAsync();
        }
        finally
        {
            CompleteUiOperationDiagnostics(
                ...
                operationName,
                operationThresholdMilliseconds,
                $"Provider={CurrentConversationModeVoiceProvider}; SessionId={realtimeSessionId}; State={CurrentConversationModeState}; IsConversationModeEnabled={IsConversationModeEnabled}; IsStarted={isRealtimeSessionStarted}");
            RefreshAllCommandStates();
            realtimeLifecycleSemaphore.Release();
        }
    }

    private Task StartConversationModeAsync()
    {
        return CurrentConversationModeVoiceProvider == ConversationModeVoiceProvider.Realtime
            ? StartRealtimeConversationModeAsync()
            : StartTtsConversationModeAsync();
    }

    private async Task StopConversationModeAsync(string reason)
    {
        HideConversationHint();
        if (IsRealtimeConversationProviderEnabled)
        {
            ClearConversationOverlayState(clearPhrases: true);
            await StopRealtimeConversationAsync(reason);
        }
        else
        {
    ...
    private async Task StartRealtimeConversationModeAsync()
    {
        ...
        {
            await CleanupRealtimeAfterFaultAsync("fatal_realtime_error");
            BackendStatusText = BackendConstants.BackendStatusUnavailable;
    ...
    private async Task<bool> SendLessonMessageAsync(string userMessage, string messageSource = ChatMessageSource.Typed)
    {
        ...
        if (nextLearnerTurnCount >= finalTurn)
        {
            ...
            if (IsTtsConversationModeActive)
            {
                ConversationLatestBotText = finalMessage;
                HideConversationHint();
                await PlayConversationModeBotVoiceAsync(botMessage);
            }
            else
            {
        ...
        try
        {
            ...
            if (IsTtsConversationModeActive)
            {
                ConversationLatestUserText = userMessage;
                HideConversationHint();
                SetConversationModeState(ConversationModeState.WaitingForAssistant, "tts_user_transcript_sent_to_lesson_chat");
            }
            LearnerTurnCount = nextLearnerTurnCount;
            ...
            if (IsTtsConversationModeActive)
            {
                ConversationLatestBotText = botReply;
                HideConversationHint();
            }
            OnPropertyChanged(nameof(LatestBotMessageText));
    ...
    private void PrepareForRealtimeConversationStartup(string reason)
    {
        ...
        StatusMessage = string.Empty;
        RefreshAvatarState();
        Debug.WriteLine($"Normal bot voice playback stopped before Conversation Mode startup: Reason={reason}; SessionId={realtimeSessionId}; AutoPlaySuppressed={IsConversationModeEnabled || IsRealtimeConversationProviderEnabled}.");
    }
    ...
    private async Task StopRealtimeConversationAsync(string reason)
    {
        ...
        Debug.WriteLine($"StopRealtimeConversationAsync requested: SessionId={realtimeSessionId}; Reason={mappedReason}; RawReason={reason}; PreviousState={previousState}; IsStarted={isRealtimeSessionStarted}; IsRecording={IsRecording}; IsSending={IsSending}; IsBotVoicePlaying={IsBotVoicePlaying}; IsRealtimePlaybackActive={IsRealtimePlaybackActive()}; RealtimePlaybackServiceActive={realtimeAudioPlaybackService.IsPlaybackActive}.");
        if (ShouldBlockRealtimeStop(mappedReason))
        {
            Debug.WriteLine($"Realtime stop/cancel blocked during assistant turn: SessionId={realtimeSessionId}; Reason={mappedReason}; RawReason={reason}; PreviousState={previousState}; IsSending={IsSending}; IsBotVoicePlaying={IsBotVoicePlaying}; IsRealtimePlaybackActive={IsRealtimePlaybackActive()}; RealtimePlaybackServiceActive={realtimeAudioPlaybackService.IsPlaybackActive}.");
            RefreshAllCommandStates();
            return;
        }
    ...
    private bool ShouldBlockRealtimeStop(string mappedReason)
    {
        if (!isRealtimeSessionStarted || !IsConversationModeEnabled)
        {
            return false;
        }

        if (IsExplicitRealtimeStopReason(mappedReason))
        {
            return false;
        }

        var assistantTurnActive = CurrentConversationModeState == ConversationModeState.PlayingAssistantAudio
            || CurrentConversationModeState == ConversationModeState.WaitingForAssistant
            || IsSending
            || realtimeAudioPlaybackService.IsPlaybackActive;
        return assistantTurnActive;
    }

    private static bool IsExplicitRealtimeStopReason(string mappedReason)
    {
        return mappedReason is "user_exit_conversation_mode"
            or "user_back"
            or "window_closing"
            or "lesson_finished"
    ...
    private static string MapRealtimeLifecycleReason(string reason)
    {
        ...
        {
            "user_exit_conversation_mode" => "user_exit_conversation_mode",
            "user_back" => "user_back",
            "window_closing" => "window_closing",
            "lesson_finished" => "lesson_finished",
            "fatal_realtime_error" or "receive_loop_failed" or "fallback_text_send_failed" => "fatal_realtime_error",
            "microphone_failure" => "microphone_failure",
            "assistant_audio_playback_failure" => "assistant_audio_playback_failure",
            "assistant_audio_delta" => "assistant_audio_playback_started",
            "assistant_turn_completed" or "assistant_playback_completed" or "assistant_turn_completed_waiting_for_playback" or "assistant_turn_completed_no_playback" => "assistant_audio_playback_completed",
    ...
    private async Task CleanupRealtimeAfterFaultAsync(string reason)
    {
        ...
        if (ShouldBlockRealtimeStop(mappedReason))
        {
            Debug.WriteLine($"Realtime fault cleanup stop/cancel blocked during assistant turn: SessionId={realtimeSessionId}; Reason={mappedReason}; RawReason={reason}; State={CurrentConversationModeState}; IsSending={IsSending}; IsBotVoicePlaying={IsBotVoicePlaying}; IsRealtimePlaybackActive={IsRealtimePlaybackActive()}; RealtimePlaybackServiceActive={realtimeAudioPlaybackService.IsPlaybackActive}.");
            SetConversationModeState(ConversationModeState.PlayingAssistantAudio, "blocked_fault_cleanup_during_assistant_playback");
            RefreshAllCommandStates();
            return;
        }
    ...
    private void OnRealtimeAssistantAudioChunkReceived(object? sender, AssistantAudioChunkReceivedEventArgs args)
    {
        Application.Current.Dispatcher.Invoke(() =>
        {
            ...
            {
                Debug.WriteLine($"Realtime assistant audio playback failure was non-fatal: SessionId={args.SessionId}; ResponseId={args.ResponseId}; Reason=assistant_audio_playback_failure; StopConversationModeRequested=False; {exception}");
                IsBotVoicePlaying = false;
                SetConversationModeState(isRealtimeSessionStarted ? ConversationModeState.Ready : ConversationModeState.NotStarted, "assistant_audio_playback_failure_nonfatal");
                StatusMessage = "Conversation Mode is still active, but assistant audio could not play.";
            }
    ...
    private async Task StartActiveRoleplayAfterContextSelectionAsync(string startMessage, int learnerTurnCountBefore)
    {
        ...
        RefreshAllCommandStates();
        Debug.WriteLine($"PhaseTransition SetupContextSelection -> ActiveRoleplay; SelectedContextVariantId={selectedContextVariant?.Id ?? string.Empty}; SelectedContextTitle={GetSelectedContextTitle()}; LearnerTurnCountBefore={learnerTurnCountBefore}; LearnerTurnCountAfter={LearnerTurnCount}; ConversationModeEnabled={IsConversationModeEnabled}; RealtimeStartDeferred={IsConversationModeEnabled && IsRealtimeConversationProviderEnabled}.");
        ...
        if (IsConversationModeEnabled && IsRealtimeConversationProviderEnabled)
        {
    ...
    private async Task ViewFeedbackAsync(ChatMessageViewModel? message)
    {
        ...
        {
            Debug.WriteLine("Feedback view ignored: requestedMessageId=0; requestedSourceKind=LegacyNullCommandParameter; requestedTextLength=0; resultMessageId=0; displayedUnderMessageId=0; staleResultIgnored=False.");
            return;
        }
        ...
        if (feedbackByMessageId.TryGetValue(requestedMessageId, out var cachedFeedback))
        {
            ...
            DisplayFeedbackForRequestedMessage(requestedTarget, cachedFeedback, fromCache: true);
            return;
        }
        ...
        if (feedback is not null)
        {
            ...
            DisplayFeedbackForRequestedMessage(requestedTarget, feedback, fromCache: true);
            return;
        }
        ...
        if (SelectedFeedbackMessageId != requestedMessageId)
        {
            Debug.WriteLine($"Feedback result ignored as stale: requestedMessageId={requestedMessageId}; requestedSourceKind={requestedSourceKind}; requestedTextLength={requestedTextLength}; resultMessageId={requestedMessageId}; displayedUnderMessageId={SelectedFeedbackMessageId}; staleResultIgnored=True.");
            return;
        }
    ...
    private async Task FinishLesson()
    {
        ...
        try
        {
            ...
            await StopConversationModeAsync("lesson_finished");
            CompleteLesson();
        }
        finally
        {
    ...
    private void MarkLessonCompleteAwaitingFinish()
    {
        ...
        BotStatus = BackendConstants.BotStatusReady;
        StatusMessage = AppConstants.LessonCompleteAwaitingFinishMessage;
        _ = StopConversationModeAsync("lesson_finished");
    ...
    private async Task<Feedback?> GenerateFeedbackForMessageAsync(FeedbackRequestTarget target)
    {
        try
        {
    ...
    private LessonChatBackendRequest BuildLessonFeedbackRequest(FeedbackRequestTarget target)
    {
