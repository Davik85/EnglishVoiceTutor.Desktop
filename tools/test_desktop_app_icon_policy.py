...
def main() -> None:
    ...
    assert_contains(inno, '#define InstalledShortcutIconFile "{app}\\Assets\\Branding\\app-icon-" + AppVersion + ".ico"', "version-specific installed shortcut icon constant")
    ...
    assert_contains(inno, 'Source: "{#AppIconFile}"; DestDir: "{app}\\Assets\\Branding"; Flags: ignoreversion', "canonical installed icon file")
    assert_contains(inno, 'Source: "{#AppIconFile}"; DestDir: "{app}\\Assets\\Branding"; DestName: "app-icon-{#AppVersion}.ico"; Flags: ignoreversion', "version-specific installed shortcut icon file")
    assert_regex(inno, r'Name:\s+"\{group\}\\Language Voice Tutor".*IconFilename:\s+"\{#InstalledShortcutIconFile\}"', "Start Menu version-specific shortcut icon")
    assert_regex(inno, r'Name:\s+"\{commondesktop\}\\Language Voice Tutor".*IconFilename:\s+"\{#InstalledShortcutIconFile\}"', "common desktop version-specific shortcut icon")
    ...
    assert_contains(inno, 'Type: files; Name: "{app}\\Assets\\Branding\\app-icon-*.ico"', "narrow old version-specific shortcut icon cleanup")
    assert_not_contains(inno, 'Type: files; Name: "{app}\\Assets\\Branding\\app-icon.ico"', "canonical installed icon cleanup")
