...
REQUIRED_BY_DOCUMENT = {
    "README.md": ["Windows Direct Release 1.2", "0.1.35-backend.134", "does not silently auto-update"],
    "docs/CURRENT_STATE.md": ["Windows Direct Release 1.2", "LanguageVoiceTutorSetup-1.2.exe", "0.1.35-backend.134", "Google Play remains disabled", "no backend deployment or migration occurred"],
    "docs/NEXT_STEPS.md": ["current Windows direct public release is `1.2`", "LanguageVoiceTutorSetup-1.2.exe", "endpoint remains disabled"],
    "docs/TESTER_RELEASE.md": ["Windows Direct Release 1.2", "LanguageVoiceTutorSetup-1.2.exe", "0.1.35-backend.134"],
    "docs/WINDOWS_INSTALLER_RELEASE_FLOW.md": ["LanguageVoiceTutorSetup-1.2.exe", "0.1.35-backend.134", "manual-confirmation"],
    "docs/WINDOWS_INSTALLER_UPDATE_FLOW.md": ["LanguageVoiceTutorSetup-1.2.exe", "version: 1.2", "does not silently auto-update"],
    "docs/WINDOWS_RELEASE_SERVER_UPLOAD.md": ["version: 1.2", "LanguageVoiceTutorSetup-1.2.exe", "no backend deployment or migration"],
    "docs/LOCAL_RELEASE.md": ["$ReleaseVersion = \"<release-version>\"", "Windows Direct 1.2 passed"],
}
...
FORBIDDEN_PATTERNS = [
    ...
    (re.compile(r"(?:current|active|latest verified|last verified public snapshot)[^\n]*(?:Windows Direct (?:Release )?1\.0|Windows Direct (?:Release )?1\.1|version[:= ]+1\.1|LanguageVoiceTutorSetup-1\.1\.exe)", re.I), "old Windows release current wording"),
    (re.compile(r"Current public Windows Direct\s+`?1\.1`?", re.I), "Windows 1.1 current wording"),
    ...
    (re.compile(r"Windows (?:Direct )?1\.2[^\n]*(?:backend deployment|migration).*(?:required|performed)", re.I), "Windows 1.2 backend or migration claim"),
...
def main() -> int:
    ...
    for needle in [
        "LanguageVoiceTutorSetup-1.2.exe",
