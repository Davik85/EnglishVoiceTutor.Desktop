...
require('HideConversationHint();' in vm_text[vm_text.find('private async Task StartVoiceRecordingAsync'):vm_text.find('private async Task StartRealtimeVoiceRecordingAsync')], "Conversation hint must hide on default TTS record start")
require('HideConversationHint();' in vm_text[vm_text.find('private async Task StartRealtimeVoiceRecordingAsync'):vm_text.find('private async Task StopVoiceRecordingAsync')], "Conversation hint must hide on realtime record start")
require('StopConversationModeAsync("user_exit_conversation_mode")' in command_body('ToggleConversationMode') and 'ClearConversationOverlayState(clearPhrases: true);' in vm_text[vm_text.find('private async Task StopConversationModeAsync'):vm_text.find('private Task StartTtsConversationModeAsync')], "Conversation hint must clear on Conversation Mode exit/back button")
require('ConversationLatestUserText = trimmedTranscriptionText' in vm_text and 'HideConversationHint();' in vm_text[vm_text.find('ConversationLatestUserText = trimmedTranscriptionText') - 200:vm_text.find('ConversationLatestUserText = trimmedTranscriptionText') + 300], "Conversation hint must hide when default TTS user transcript arrives")
require('HideConversationHint();' in vm_text[vm_text.rfind('OnRealtimeUserTranscriptDeltaReceived'):vm_text.rfind('OnRealtimeUserTranscriptCompleted')], "Conversation hint must hide when realtime user transcript arrives")
require('ConversationLatestBotText = botReply' in vm_text and 'HideConversationHint();' in vm_text[vm_text.find('ConversationLatestBotText = botReply') - 200:vm_text.find('ConversationLatestBotText = botReply') + 300], "Conversation hint must hide when default TTS bot reply arrives")
require('HideConversationHint();' in vm_text[vm_text.rfind('OnRealtimeAssistantTranscriptDeltaReceived'):vm_text.rfind('OnRealtimeAssistantTurnCompleted')], "Conversation hint must hide when realtime bot transcript arrives")
