...
def test_backend_level_settings_drive_lesson_behavior() -> None:
    ...
    require(limits, "CmsLevelProfiles.Resolve", "backend default fallback in lesson limit helper")
    require(limits, "request.SoftWrapUpAfterUserTurn > 0", "resolved request wrap threshold is honored")
    require(limits, "request.FinalMessageAtUserTurn > 0", "resolved request final threshold is honored")
    require(runtime, "ApplyPublishedSnapshotLevelProfiles", "runtime applies CMS published levels into CMS scenarios")
