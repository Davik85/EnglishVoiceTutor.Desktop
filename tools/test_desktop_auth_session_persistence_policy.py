...
def main() -> int:
    ...
    assert_contains(docs, "update/reinstall should preserve auth session", "session preservation docs")
    ...
    assert_contains(docs, "Auth session persistence works across app restart and Windows restart", "tester-readiness docs")
    assert_contains(docs, "Same-version install asks for reinstall confirmation", "same-version reinstall docs")
    assert_contains(docs, "Check for updates", "manual update UI docs")
    assert_contains(docs, "latest.json", "latest.json docs")
    assert_contains(docs, "SHA-256", "SHA-256 docs")
    assert_contains(docs, "does not silently auto-update", "no silent auto-update docs")
    assert_contains(docs, "small controlled external tester", "external handoff docs")
