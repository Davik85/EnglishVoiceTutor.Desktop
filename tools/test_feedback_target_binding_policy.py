...
def main() -> int:
    ...
    code_behind = read("Views/LessonChatView.xaml.cs")
    ...
    assert_contains(xaml, "x:Name=\"FeedbackPanelCard\"", "feedback panel has a card-level click target")
    assert_contains(xaml, "PreviewMouseLeftButtonUp=\"FeedbackPanelCard_PreviewMouseLeftButtonUp\"", "feedback panel card-wide close handler")
    assert_contains(xaml, "Cursor=\"Hand\"", "feedback panel card communicates clickability")
    assert_contains(code_behind, "FeedbackPanelCard_PreviewMouseLeftButtonUp", "feedback card-wide close handler implementation")
    assert_contains(code_behind, "CloseFeedbackCommand.Execute(null)", "feedback card-wide close uses existing close command")
    assert_contains(code_behind, "IsEventFromFeedbackInteractiveControl", "feedback card-wide close protects interactive controls")
