...
TIMELINE = ROOT / "backend" / "EnglishVoiceTutor.Api" / "Services" / "Subscriptions" / "PremiumCoverageTimeline.cs"
...
def main() -> None:
    ...
    need(service, "PremiumCoverageTimeline.Calculate", "shared account-wide continuous coverage calculation")
    need(timeline, "if (interval.StartsAtUtc > endsAtUtc)", "gap stops continuous coverage chain")
    need(timeline, "endsAtUtc = interval.EndsAtUtc.Value", "continuous queued entitlements extend coverage end")
    need(timeline, "if (!interval.EndsAtUtc.HasValue)", "indefinite entitlement handling")
