...
At this historical planning checkpoint, runtime processing was dormant because Google Play was disabled, and the listed enablement/rollout work was pending at that time. Current production state is recorded in [CURRENT_STATE.md](CURRENT_STATE.md): backend `.151` has Google Play Billing, RTDN, and reconciliation enabled, and Android v8 is public in Production. Paddle remains a provider beside Google Play through the same backend entitlement source of truth; a user with active Paddle Premium must not be prompted into a duplicate Google Play subscription or have valid Paddle Premium replaced or shortened.
