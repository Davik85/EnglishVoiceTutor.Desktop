...
The public tester Windows direct manifest baseline must be checked from the live website `latest.json`. Last verified public snapshot: `latest.json` pointed to `LanguageVoiceTutorSetup-0.1.36-tester.16.exe` with `version` set to `0.1.36-tester.16`, `backendBaseUrl` set to `https://api.languagevoicetutor.com`, `minimumSupportedVersion` set to `0.1.36-tester.16`, and `updateMode` set to `manual-confirmation`. Treat this as a controlled tester/direct Windows release baseline only; do not describe any future local build as public/live unless the website `latest.json` points to it over HTTPS.
