...
# Setup localization foundation

Scenario `DefinitionJson` may optionally contain `setupLocalizations`, keyed by canonical study-language IDs. Each entry contains an authored `setupMessageTemplate` and `contextVariantTitles` keyed by stable context-variant IDs. Runtime adds request-specific `localizedSetup`; authenticated study language is resolved from backend user settings. English projects canonical content, while missing non-English data returns `status: "incomplete"` without English fallback. Desktop and Mobile still use their existing local setup rendering; content population and all-language publication enforcement are future work. No migration, CMS republish, or deployment was performed for this foundation.
