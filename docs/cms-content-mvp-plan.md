...
Step 5D-6d completed Admin CMS refresh resilience and unsaved-change protection for the development/admin-only `/admin/` shell. Admin refresh no longer logs out the admin because refresh authentication uses the existing admin-only HTTP-only cookie, while the admin JWT remains memory-only in JavaScript. Browser Web Storage is not used: no `sessionStorage`, no `localStorage`, and no IndexedDB.
...
5. Add Admin content API for draft read/update operations plus server-side validation and preview summaries. **Implemented in Step 5D-4 as development/admin-only backend endpoints.**
...
6. Add publish/version/rollback workflow and audit log. **Implemented in Step 5D-5 as development/admin-only backend endpoints.**
...
A backend runtime lesson-content read service now provides the safe handoff point from static JSON to CMS published snapshots. The default remains static JSON. CMS runtime mode requires `CmsContent:UsePublishedSnapshotForRuntime=true`; snapshot reads still require `CmsContent:ReadPublishedSnapshotEnabled=true`; `CmsContent:ContentPackSlug` defaults to `static-json-v1`; `CmsContent:FallbackToStaticJson` controls whether invalid/missing CMS content falls back to packaged static JSON. The runtime service maps published topics, scenarios, prompt templates, and tutor behavior profiles into the runtime content shape and validates the current baseline counts (6 topics, 26 scenarios, 3 prompt templates, 2 tutor behavior profiles) plus required fields. It reads only published snapshot JSON and never draft rows, preserving immutable publish semantics. Scenario `DefinitionJson` remains present on published scenarios to preserve structured editor output and unknown advanced JSON fields. `/api/admin/dev/cms/runtime-content/status` is the admin/development diagnostic for source, version, hash, counts, fallback, validation, and bounded errors/warnings. The local runtime path and existing Admin CMS smoke coverage are verified, but external tester handoff remains paused until the development/admin CMS MVP usability refinements are accepted. Production RBAC and critical-change approval remain future work.


## Next CMS refinement after latest completed step

The latest development/admin Admin CMS Content step is complete for structured scenario editing, draft-save publish guidance, required publish summaries with clearer local validation errors, smoke/test audit filtering, and the confirmed local CMS runtime published-snapshot read path. This is still a development/admin CMS MVP, not production CMS/RBAC readiness, not external tester handoff readiness, and not public release readiness.

The next recommended implementation step is Admin CMS scenario editor usability refinement, not billing:

1. Group structured scenario fields into clearer collapsible sections.
2. Add concise helper text for scenario fields so content editors understand the learner/runtime effect of each field.
3. Improve long scenario editing ergonomics for large text areas and repeated field groups.
