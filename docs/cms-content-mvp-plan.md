...
As of 2026-06-11 after `0.1.26-tester.1`, CMS/Admin is connected and `static-json-v1` is initialized as Draft/admin content. Static JSON stays the learner runtime default unless the CMS published snapshot runtime flag is explicitly enabled after validation. The private tester/direct Windows release is validated, but broad public release readiness still depends on clean-machine smoke, controlled tester feedback, operational hardening, production RBAC/approval, and deferred billing work.
...
The CMS MVP includes an admin-only **Initialize from static JSON** foundation for `static-json-v1`. For the current `0.1.26-tester.1` state, `static-json-v1` has been initialized as Draft/admin content; the same action remains the documented first-setup path for future clean environments when the Admin CMS database has no initialized draft/content pack yet. The action imports the current packaged static JSON lesson topics, scenarios, prompt templates, tutor profiles, and available study-language metadata references into the current CMS draft/admin model where supported.
