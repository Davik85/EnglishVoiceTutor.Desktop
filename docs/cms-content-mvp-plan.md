...
Production closeout: backend `0.1.35-backend.136` is active with `.135` retained as rollback, and CMS published version `51` is the valid `CmsPublishedSnapshot` runtime source (`fallbackUsed=false`). The publication contains all 26 active scenarios, 130 localized setup-message templates, and 625 localized context titles; no migration was required. For an existing `static-json-v1` draft created before packaged setup localizations, use the CMS Overview sequence: Preview localized setup import → Apply localized setup to draft → review the five full-message fields → Validation → Publish. Preview is read-only and reports exact scenario/template/title counts. Apply repeats the full safety check inside its protected transaction; malformed or differing existing localization data causes zero writes, and concurrent editing requires the administrator to rerun Preview. It adds only absent `setupLocalizations` language blocks, preserves all other JSON properties, creates no version or published snapshot, and does not change learner runtime.
...
Runtime learner behavior now uses the CMS published snapshot in the controlled tester environment. The CMS read path remains controlled by configuration and backed by static JSON fallback. Production RBAC, role-based approval, production billing operations, and full external tester handoff remain future work. CMS draft-save audit logging is implemented for successful Save draft operations; the later critical-change approval workflow should wait until production roles exist.
...
Compatibility remains intentionally safe: existing CMS scenario rows without `DefinitionJson` can still be viewed through an internally marked fallback JSON built from the previously stored scenario fields, while the static import smoke path populates real `DefinitionJson` for the baseline. Runtime learner behavior now targets the CMS published snapshot when enabled, valid, and effectively active; static JSON fallback remains available for emergency safety. Lesson JSON files, prompt source files, tutor source files, desktop UI, billing, Paddle, subscriptions, entitlements, payments, password reset, and support-user workflows are outside this step.
...
As of the 2026-06-12 documentation snapshot, CMS/Admin is connected and `static-json-v1` is initialized as Draft/admin content. CMS published snapshot is the intended learner runtime source after validation and publish; static JSON is the emergency fallback and initialization source. The private tester/direct Windows release must be verified from live `latest.json`; local build artifacts alone are not public/live, but broad public release readiness still depends on clean-machine smoke, controlled tester feedback, operational hardening, production RBAC/approval, and deferred billing work.
...
3. Import current JSON content into CMS draft/published seed or migration path without changing current JSON. **Implemented in Step 5D-2 as a bootstrap-admin static import foundation.**
   ...
   - Runtime lesson loading now targets the CMS published snapshot when enabled, valid, and effectively active; packaged static JSON remains the initialization and emergency fallback source.
4. Add backend published-content read path with static JSON fallback. **Implemented in Step 5D-3 as a bootstrap-admin status and service read path.**
   ...
   - Safe config defaults are `CmsContent:ReadPublishedSnapshotEnabled=true`, `CmsContent:UsePublishedSnapshotForRuntime=true`, `CmsContent:ContentPackSlug=static-json-v1`, and `CmsContent:FallbackToStaticJson=true`; fallback remains available but should be visible as an attention state when used.
   ...
   - Runtime lesson loading now uses CMS published snapshot for controlled tester lessons; this earlier step prepared the read/status path and did not add public learner-facing CMS endpoints.
...
6. Add publish/version/rollback workflow and audit log. **Implemented in Step 5D-5 as bootstrap-admin backend endpoints.**
   ...
   - Runtime lesson loading now uses CMS published snapshot for controlled tester lessons; static JSON fallback remains available.
...
The action is idempotent and safe: it creates the missing content pack, preserves existing draft content instead of blindly overwriting it, does not publish automatically, and does not switch runtime. Runtime uses CMS published snapshot when `CmsContent__UsePublishedSnapshotForRuntime=true`, `CmsContent__ReadPublishedSnapshotEnabled=true`, and a valid published snapshot exists.
...
- Deployed verification is on backend `0.1.35-backend.11`. Backend health is green, database health is green, build is green, Admin shell audit is green, and the EF model check reports no pending model changes.
...
CMS published-snapshot runtime is the intended learner source. Keep static JSON fallback configured for emergency rollback and initialization, and make any fallback usage visible in Admin CMS diagnostics.

## Step 5D-7 update — Runtime status readiness without default enablement

Step 5D-7 prepared the published-snapshot runtime read path for controlled validation before the runtime milestone. The existing runtime service reads immutable published snapshots when CMS runtime is enabled and falls back to static JSON only when configured and needed for safety. If `CmsContent:UsePublishedSnapshotForRuntime` is false, Admin CMS should treat the resulting static JSON runtime source as an attention state rather than the normal learner model.
...
`-GenerateServerValidationPlan` prints the exact temporary flags for an explicitly approved controlled window only: `CmsContent__UsePublishedSnapshotForRuntime=true`, `CmsContent__ReadPublishedSnapshotEnabled=true`, `CmsContent__ContentPackSlug=static-json-v1`, and `CmsContent__FallbackToStaticJson=true`. It also prints the operator checklist and rollback path. After applying the flags manually and restarting backend during the controlled window, runtime status must show `effectiveSource=CmsPublishedSnapshot`, `validationSuccess=true`, 6 topics, 26 scenarios, 3 prompt templates, and 3 tutor behavior profiles. Rollback is to disable/remove the CMS runtime flags, restart backend, and confirm `effectiveSource=StaticJson`.

CMS published-snapshot runtime is active for controlled tester lessons; broad public production release still requires a separate approval. Billing/Paddle remains deferred and is not involved; no production config file, public Windows `latest.json`, installer behavior, desktop runtime behavior, deployment script, lesson JSON, or EF migration is changed by this tooling/documentation step.
...
# Setup localization production content and publication gate

Each of the 26 active canonical scenario definitions now contains `setupLocalizations` for `fr`, `de`, `pt`, `es`, and `it`: 130 approved localized setup-message templates and 625 localized context titles. The data remains authored in canonical scenario JSON; English setup messages, English context titles, stable context IDs, and scenario behavior remain canonical and unchanged. Runtime adds the response-only `localizedSetup` projection from the authenticated learner's backend study-language setting.

Draft save and editing remain permissive for incomplete localization work. Publishing a new snapshot now rejects an incomplete active scenario when a required language, template, exact stable context ID, non-blank localized title, required placeholder, or placeholder-safe template is missing or invalid. The shared study-language catalog is the required-language source. Legacy published snapshots without `setupLocalizations` remain readable for runtime and rollback; the new completeness rule applies only to creating a new publication. No database migration, deployment, CMS import/publication, or production operation was performed.

Desktop and Mobile still use their existing local first-message rendering and have not switched to `localizedSetup`. The rollout sequence remains: (1) review and commit; (2) push; (3) deploy a new backend release; (4) verify backend health; (5) import canonical content into the CMS draft; (6) validate and publish the CMS snapshot; (7) verify production `localizedSetup` responses; (8) switch Desktop; then (9) switch Mobile separately.

The scenario editor now presents five normal multiline fields for the complete French, German, Portuguese, Spanish, and Italian learner-visible first messages, immediately after the existing English field. Structured Save draft merges their values into the matching `setupLocalizations.<language>.setupMessageTemplate` paths of the same `DefinitionJson`; Advanced JSON Save draft preserves the entered JSON document as authoritative, and the existing successful-save reload refreshes the visible fields from saved JSON. The normal form does not split messages into Goal, situation, or instruction fields and does not expose `contextVariantTitles`; those mappings remain internal Advanced JSON data. Draft editing remains permissive and the existing publication-completeness validation remains authoritative. No deployment or CMS publication has occurred, and Desktop and Mobile remain unchanged.
