...
- Deployed verification is on backend `0.1.35-backend.8`. Backend health is green, database health is green, build is green, Admin shell audit is green, and the EF model check reports no pending model changes.
...
The Admin CMS now includes a read-only **Runtime content status** panel backed by `GET /api/admin/dev/cms/runtime-status`. The response is restricted to safe metadata and bounded diagnostics; it does not expose scenario definitions, prompt bodies, tutor instruction bodies, secrets, tokens, API keys, connection strings, or auth headers. Static JSON remains the default. Controlled server validation comes first; localhost is only an explicit developer override for local backend work. The local root cause of the previous validation mismatch was an obsolete exact count of 2 tutor behavior profiles; product content legitimately has approved tutor ids `david`, `elena`, and `nelli`. Runtime validation now checks those ids and reports expected, actual, missing, unknown/extra, and duplicate tutor ids. Runtime status is green on backend `0.1.35-backend.8`; before CMS published-snapshot runtime is enabled for learners, next run controlled environment validation with `CmsContent:UsePublishedSnapshotForRuntime=true`, `CmsContent:ReadPublishedSnapshotEnabled=true`, `CmsContent:ContentPackSlug=static-json-v1`, and `CmsContent:FallbackToStaticJson=true`, then decide separately whether to enable CMS runtime for a limited learner/tester group.

## Step 5D-8 update — Operator-safe CMS published-snapshot runtime validation

Backend `0.1.35-backend.8` has clean runtime status diagnostics in the deployed Admin path: `effectiveSource=StaticJson`, `validationSuccess=Yes`, no errors, no warnings, and 3 tutor behavior profiles for the approved tutor ids `david`, `elena`, and `nelli`. StaticJson remains the validated learner default.

`tools/validate_cms_published_snapshot_runtime.ps1` provides the controlled validation path. Its default mode is read-only, calls `https://api.languagevoicetutor.com/api/admin/dev/cms/runtime-status` with an admin access token or other approved admin auth method, and verifies that `usePublishedSnapshotForRuntime=false` and learner runtime is not using CMS snapshot. The script does not change configuration.
...
CMS published-snapshot runtime must not become the learner default until controlled validation passes and a separate approval is made. Billing/Paddle remains deferred and is not involved; no production config file, public Windows `latest.json`, installer behavior, desktop runtime behavior, deployment script, lesson JSON, or EF migration is changed by this tooling/documentation step.
