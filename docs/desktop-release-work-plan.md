...
Admin CMS workflow remains safe for tester preparation: `Save draft` does not publish, and saved drafts are not runtime-visible until the existing **Versions & Publish** / **Publish current draft** confirmation flow is used. The editor now makes that path discoverable with a post-save **Go to Publish** action. CMS published snapshot is the intended learner runtime source after publishing and validation. Static JSON fallback remains available for emergency rollback and should be treated as an attention state if active.

External tester handoff remains paused even though the controlled local CMS runtime read path has now been verified. The desktop UI and packaged static lesson/prompt/tutor files remain unchanged for this step. Backend runtime content should use CMS published snapshot when `CmsContent:UsePublishedSnapshotForRuntime=true`, `CmsContent:ReadPublishedSnapshotEnabled=true`, and the snapshot is valid; static JSON fallback remains an attention state. The admin/development diagnostic `/api/admin/dev/cms/runtime-content/status` and `tools/smoke_cms_runtime_content_read.ps1` should continue to prove published snapshot reads, expected counts, valid hash, validation passed, and no fallback on the happy path before tester packaging resumes. This is not production CMS/RBAC readiness.
