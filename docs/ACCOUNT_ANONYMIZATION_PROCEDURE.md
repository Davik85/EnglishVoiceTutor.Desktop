...
> **Current approved procedure (2026-07-23).** This section supersedes earlier draft workflow, retention, external-provider, confirmation, notification, and restore-reconciliation proposals below. The complete Super-Admin workflow was deployed in historical production `0.1.35-backend.133` (rollback `.132`), including migration `20260723045852_AddAccountAnonymizationExecution`, and remains in current production `.134`. A controlled production test verified the complete operation. No second Admin, password re-entry, typed phrase, provider call, queue, email automation, or custom backup workflow is required. Active effective Premium or a current paid subscription period blocks execution until expiry; the operator reminds the customer to cancel renewal. Refunds, disputes, and chargebacks remain manual support work. The transaction removes normal learner data and access, retains unchanged local subscription/payment/billing/Paddle records, redacts support content, and leaves a non-login `deleted+<operationId>@deleted.invalid` user shell. Standard backup retention applies.

## 1. Status and scope

**Status: complete and production-verified for the approved current scope.** The learner request intake, preflight foundation, Super-Admin email-request intake, and complete Admin Shell confirmation/execution flow were deployed in `.133` and remain in current production `.134`. Migration `20260723045852_AddAccountAnonymizationExecution` is applied.
