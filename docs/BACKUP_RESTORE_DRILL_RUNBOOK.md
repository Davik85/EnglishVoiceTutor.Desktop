...
## 2026-07-01 pre-migration backup evidence for Admin auth audit

Before applying migration `20260701000000_AddAdminAuthAuditEvents`, operators created a fresh production PostgreSQL custom-format backup and reviewed SQL. Safe evidence only: backup path `/var/backups/languagevoicetutor/postgres/lvt_app_db_20260701_154405Z.dump`, size `6.4M`, and `pg_restore --list` line count `245`. The migration was applied before backend `0.1.35-backend.94`; backend health and database health later returned `200 Healthy`. Do not paste or commit backup contents, SQL dumps, secrets, env files, tokens, cookies, provider payloads, or raw user data.
