...
No Store/MSIX package command, Windows App Certification Kit command, Partner Center submission command, Store package identity, Store version mapping, Store channel build flag, or Store local-data migration process is confirmed yet. Review `docs/WINDOWS_STORE_LOCAL_DATA_AUDIT.md` and `docs/WINDOWS_STORE_CHANNEL_UPDATE_PLAN.md` before the first MSIX prototype, keep the local-data and channel/update implementation decisions pending until reviewed, implement the Store channel flag/update behavior before MSIX packaging, and do not invent or run final Store commands from this playbook until a real local MSIX prototype confirms them and `docs/WINDOWS_STORE_RELEASE_PLAN.md` is updated.
