...
## Website tab boundary after 2026-06-25 rollout

Website policy/legal/support/pricing management is intentionally separate from the learner/runtime `CMS Content` workspace. The Website area is a top-level Admin Shell tab in the left navigation, not a `CMS Content` sub-tab. The current Website tab is read-only planning/status only: it does not save drafts, publish content, edit public pages, change runtime learner content, or connect public rendering.

The Website CMS database foundation now exists through `website_cms_sections`, and production migration `20260625090000_AddWebsiteCmsLegalContentFoundation` has been applied. Public rendering remains static from `site/public/`, no draft Website CMS content may be served publicly, and live Paddle remains a separate readiness step. Website CMS must not store or display Paddle secrets, webhook secrets, API keys, JWT keys/signing keys, raw provider payloads, customer IDs, transaction IDs, or subscription IDs.
...
## 2026-06-25 update — Admin CMS Overview runtime-source clarity

Backend `0.1.35-backend.48` is deployed for the current Admin CMS clarity state, and Windows direct tester `0.1.36-tester.30` is published. The Admin CMS Overview separates CMS content pack/seed identity, draft workspace status, published snapshot status, actual learner runtime source, emergency static JSON fallback enabled, and currently using static JSON fallback. `static-json-v1` / `Static JSON Baseline` is the CMS content pack and seed identity; it is not proof that learners are actively using static JSON fallback. Healthy controlled tester runtime is `Actual learner runtime source = CmsPublishedSnapshot`, `Validation success = Yes`, `Currently using static JSON fallback = No`, and a published version exists. Static JSON remains initialization/emergency fallback only, not the normal production behavior tuning surface. Normal prompt, scenario, level, and tutor behavior tuning should use CMS draft, validation/preview, publish, and restore workflows.
