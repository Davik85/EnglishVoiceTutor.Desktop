...
Publishing AI model settings affects new backend AI requests without a desktop release because the desktop continues to call backend endpoints and does not choose OpenAI model IDs. If the CMS file is missing, unreadable, or contains invalid active values, backend runtime logs a safe warning and falls back to the current safe defaults so lessons are not broken by CMS storage problems. Do not change production models casually; after publishing model changes, run a small new lesson smoke test covering text reply, voice transcription, and speech playback.
