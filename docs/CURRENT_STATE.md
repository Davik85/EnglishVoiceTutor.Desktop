...
Backend `0.1.35-backend.57` is deployed at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.57`; health and database health returned `200 Healthy`. Production smoke on `platform_status` saved a draft, changed review status to `legal_approved`, validated and previewed the draft, published it into internal `PublishedBody`, and confirmed `PublishedAtUtc` was set while public rendering did not change. Smoke data was manually cleaned up because rollback/unpublish is not implemented yet. Post-cleanup DB verification confirmed 9 rows, all `ReviewStatus=not_started`, all `DraftBody` lengths `0`, and `PublishedBody` null / `has_published=false` for all rows. Fresh service logs showed no website-cms errors, permission denied errors, exceptions, failures, or `500` responses.

Website CMS now has an admin-only explicit publish workflow into internal `PublishedBody` storage, but there is still no Website CMS public rendering, no public unauthenticated Website CMS route, and no live Paddle enablement. Public rendering remains static `site/public`; public rendering integration is not implemented. Final legal/seller/support/pricing copy still requires owner/legal approval.
