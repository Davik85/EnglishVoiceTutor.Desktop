...
This is repository state, not deployment evidence. Production remains `0.1.35-backend.133`; the current backend release is `0.1.35-backend.133`, and commits after `.133` do not change that release claim. Google Play Subscription creation, ownership/persistence transaction orchestration, account-status multi-provider selection, credentials, package/product configuration, Premium activation, acknowledgement, replacement-token reconciliation, RTDN, Mobile connection, sandbox testing, migration application, and deployment remain pending. Paddle checkout, webhook processing, Premium activation/cancellation/refund behavior, and Desktop Premium-expiry display remain the working website/Desktop path. Google Play must later join—not replace—that shared entitlement source of truth.
