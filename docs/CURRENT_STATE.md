...
4. Logging/release-readiness checks for remaining Admin operations and paid-launch evidence.
5. Admin auth audit first production slice is complete for `admin_login_success`, `admin_logout`, `admin_login_failed`, and `disabled_admin_login_denied`; session expiration audit persistence remains pending until separately implemented/verified.
6. Controlled Paddle live payment/webhook/Premium activation and desktop cancel-renewal validation are completed; refund, chargeback, customer portal, and broad paid launch remain pending.
...
powershell -ExecutionPolicy Bypass -File .\scripts\upload-backend-linux-release.ps1 -Version 0.1.35-backend.95
```
...
Production Admin RBAC / persistent role management is completed after backend release `0.1.35-backend.95`, deployed by the normal backend package/upload flow. The production backend `current` symlink was verified at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.95`; `/health` and `/api/health/database` returned `200 Healthy`. No EF migrations were added or run for this RBAC stage. Windows installer release files were not changed.
...
The `0.1.36-tester.31` Windows direct tester release has been built, uploaded, and verified. The user confirmed the newly uploaded build works and that the manual-confirmation update flow works on other devices. Backend deployment was not part of this desktop polish release; production backend remains healthy at `0.1.35-backend.95`, and no database migrations were added or run.
...
- Backend release in tracked release docs: `0.1.35-backend.95`. The live `/opt/languagevoicetutor/backend/current` symlink was manually verified with `ssh lvt-server "readlink -f /opt/languagevoicetutor/backend/current"` and resolved to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.95`; `/health` and `/api/health/database` were also verified healthy. Backend .95 was deployed by the normal backend package/upload flow for the Admin Activity and emergency Premium revoke stage; this documentation task did not deploy backend code.
...
Partially ready: Windows public installer release because signing and wider smoke/feedback remain; website/legal pages because owner/legal final review remains; AI tutor quality because CMS content approval and tester feedback remain. Backend operations remain controlled/manual: current production is documented as `0.1.35-backend.95`, with deploys, health checks, database health checks, and migrations kept as separate operations.

Blocked before broad public paid release: code signing for the direct installer, direct installer clean-machine/update smoke, refund/chargeback/customer portal checks, final website/legal/support/pricing approval, and explicit release decision after controlled tester feedback. Controlled Paddle live payment/Premium activation and cancel-renewal behavior are completed, but they are not a broad launch decision.
...
Paddle approved the website, backend live checkout code is deployed in production `0.1.35-backend.95`, `/pay.html` and `/paddle.public.json` are published under the real nginx root, and live server-side Paddle config is present in `/etc/languagevoicetutor/backend.env`. The controlled 2026-07-02 live payment/webhook/Premium activation path completed for the expected Language Voice Tutor Pro monthly price, and desktop cancel-renewal behavior was verified. Windows direct tester release remains `0.1.36-tester.31`; AI Models persistent storage is verified and untouched. Store/MSIX remains discontinued; active Windows distribution remains Direct EXE/Inno.
...
Production backend current release is `0.1.35-backend.95`; the `current` symlink was verified at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.95`, `languagevoicetutor-backend.service` is active/running, `/health` returns `200 Healthy`, and `/api/health/database` returns `200 Healthy`. Backend .95 was deployed through the normal backend package/upload flow. The deploy script did not run EF migrations, and Windows installer files were not changed.
...
- Controlled Paddle live payment/webhook/Premium activation and desktop cancel-renewal validation were completed on 2026-07-02. Refund, chargeback, customer portal, and broad paid launch remain pending. Session expiration audit persistence remains pending and must not be marked complete.

## Admin auth audit persistence production verification (2026-07-01)

- Migration `20260701000000_AddAdminAuthAuditEvents` was applied before backend `0.1.35-backend.95` deployment, after fresh backup creation and SQL review.
...
- Production backend `0.1.35-backend.95` is deployed successfully; `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.95`, `languagevoicetutor-backend.service` is active/running, `/health` returns `200 Healthy`, and `/api/health/database` returns `200 Healthy`.
...
Backend `0.1.35-backend.98` was deployed and healthy, and the operator-only command ran correctly through `systemd-run` with the backend environment file for `evt_01kwhgmvh1v9k8ve70gvnfeskm`, but it returned `Result=Blocked` / `BlockReason=reconciliation_blocked` even though it found the stored `adjustment.updated` event, resolved the user through payment history, detected a full refund, and found one active Premium entitlement candidate. Root cause: `.98` reprocess still depended on the old reconciliation pipeline/state for an event already blocked/skipped under older code. Backend `.99` fixes the explicit operator-only recovery path to revoke after all safety checks pass without another live payment, refund, or Paddle replay. Broad public paid launch remains pending until the `.99` command returns `Revoked` or `AlreadyRevoked` and Desktop/Admin confirm Premium is inactive. Customer portal remains pending unless separately verified. Direct installer code signing remains pending.
