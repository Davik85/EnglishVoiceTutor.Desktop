...
- Backend: production is deployed and healthy at `https://api.languagevoicetutor.com`; current backend release is `0.1.35-backend.115`.
...
Backend `0.1.35-backend.115` is the current production backend; `0.1.35-backend.114` is the previous backend release. The live `current` and `previous` symlinks are the source of truth for these values. Backend `0.1.35-backend.114` was already active before this deployment, so it must not be described as containing the `.115` structured-output fix. Deployment completed successfully through the existing `scripts/upload-backend-linux-release.ps1` flow, `languagevoicetutor-backend.service` is active, public `/health` returned HTTP 200, and public `/api/health/database` returned HTTP 200 with `canConnect=true`. No EF migration or database schema change was required or run. Website and Windows installer files were not deployed.
...
- Backend release in tracked release docs: current production is `0.1.35-backend.115`, previous is `0.1.35-backend.114`, and the live `/opt/languagevoicetutor/backend/current` symlink resolves to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.115`; `/health` and `/api/health/database` are verified healthy. Backend .99/.108/.112 references are historical and not current production unless a section is explicitly documenting those older releases.
...
Production backend current release is now `0.1.35-backend.115`; the `current` symlink was verified at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.115`, previous release is `/opt/languagevoicetutor/backend/releases/0.1.35-backend.114`, `languagevoicetutor-backend.service` is active, `/health` returns HTTP 200, and `/api/health/database` returns HTTP 200 with `canConnect=true`. No EF migration or database schema change was required for the `.115` voice scenario structured-output validation fix, and website/Windows installer files were not changed.
