...
## Website CMS validation/preview/review current state (2026-06-27)

Website is now a top-level Admin Shell tab, not a `CMS Content` sub-tab. The tab loads admin-only metadata, can initialize missing `website_cms_sections` rows, can select/load section detail, validate the stored draft, open an admin-only simple-text draft preview, update safe non-published review statuses with required change reason, and can save admin-only draft fields: `DraftBody`, `InternalNotes`, `EffectiveDate`, `ReviewStatus`, and required `ChangeReason`. Save and validation use `WebsiteCmsContentGuard` to block obvious secret-like/private/provider identifiers. Validation reads the stored draft and does not write to the database; preview is admin-only safe text; review-status changes are internal only and do not update `PublishedBody` or `PublishedAtUtc`. `CMS Content` remains focused on learner/runtime content packs.

The Website CMS backend foundation exists through the `website_cms_sections` table and migration `20260625090000_AddWebsiteCmsLegalContentFoundation`. Production DB rollout was applied manually from reviewed SQL; production `__EFMigrationsHistory` contains the migration, and indexes `IX_website_cms_sections_ReviewStatus` and `IX_website_cms_sections_SectionKey` plus `PK_website_cms_sections` exist. Production contains 9 rows for `seller_company`, `support`, `pricing`, `terms`, `privacy`, `refunds`, `cancellation`, `ai_data_disclosures`, and `platform_status`.

Backend `0.1.35-backend.56` is deployed, `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.56`, and health checks returned `200 Healthy` for `https://api.languagevoicetutor.com/health` and `https://api.languagevoicetutor.com/api/health/database`. Production smoke on `platform_status` saved a temporary draft, validation returned valid, the admin-only preview displayed the draft, review status changed to `owner_review_needed`, smoke data was cleared, and `ReviewStatus` was restored to `not_started`. After cleanup, DB verification confirmed 9 rows, `ReviewStatus=not_started` for all rows, `DraftBody` length `0` for all rows, and `PublishedBody` null / `has_published=false` for all rows. Fresh service logs showed no new `website-cms` errors, permission-denied errors, exceptions, failures, or `500` responses.

The static public website remains the actual public rendering source. Production public website root is `/var/www/languagevoicetutor/site`; the accidental parent-directory upload was quarantined at `/var/www/languagevoicetutor/_mistaken_static_upload_20260625`. Saving, validating, previewing, or changing Website CMS review status does not publish content, does not update public website rendering, does not modify `site/public/`, and does not imply legal approval. Admin-only publish can copy an approved non-empty DraftBody into internal PublishedBody with PublishedAtUtc, but no draft or published Website CMS content is served publicly, and live Paddle is not enabled. Final legal/seller/support/pricing values still need owner/legal approval.

## Current production backend state

Current state: verified production backend snapshot is `/opt/languagevoicetutor/backend/releases/0.1.35-backend.56`, and `/opt/languagevoicetutor/backend/current` points to that release. Verify the live value with the server symlink command before calling it current.
...
Current state: backend production deploy `0.1.35-backend.56` is complete, `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.56` at last verification. Production migration `20260620165657_AddAdminRoleAssignmentPersistence` has been applied. Production now has the Admin RBAC persistence tables `admin_users`, `admin_user_roles`, and `admin_role_assignment_events`.
...
Phase 5C production logging hardening was first deployed on backend `0.1.35-backend.40`, was production-verified on backend `0.1.35-backend.50`, and is retained in current backend `0.1.35-backend.56`. The 2026-06-24 Phase 5C verification returned `200 OK` for health/database health and 0 lines for the bounded sensitive/EF SQL grep set: `Microsoft.EntityFrameworkCore.Database.Command`, `SELECT`, `INSERT`, `UPDATE`, `PasswordHash`, `TokenHash`, `RawPayload`, and `SignatureHeader`. The later `0.1.35-backend.56` rollout added the Website CMS foundation and included a separately applied Website CMS migration; production/live Paddle readiness remains deferred, and broad public production readiness is still not claimed.
...
Website CMS can now manage admin-only draft workflow for initialized sections. The Admin Website tab supports metadata overview, missing-section initialization, section detail, draft-body save, stored-draft validation, admin-only safe-text preview, and internal review-status changes with required `ChangeReason`. Save and validation use `WebsiteCmsContentGuard`; validation reads the stored draft and does not write to the database. Preview is admin-only and does not publish or update public rendering. Review-status changes are internal only and do not update `PublishedBody` or `PublishedAtUtc`; only the explicit admin-only publish action can copy `DraftBody` to internal `PublishedBody`.

Backend `0.1.35-backend.56` is deployed at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.56`; health and database health returned `200 Healthy`. Production smoke on `platform_status` saved a draft, returned valid validation, displayed the admin-only preview, changed review status to `owner_review_needed`, cleared smoke data, and restored `ReviewStatus` to `not_started`. Post-cleanup DB verification confirmed 9 rows, all `ReviewStatus=not_started`, all `DraftBody` lengths `0`, and no published bodies. Fresh service logs showed no website-cms errors, permission denied errors, exceptions, failures, or `500` responses.

Website CMS now has an admin-only explicit publish workflow into internal PublishedBody storage, but there is still no Website CMS public rendering, no public unauthenticated Website CMS route, and no live Paddle enablement. Public rendering remains static `site/public`. Draft copy is not final legal advice or legal approval; final legal/seller/support/pricing copy still requires owner/legal approval.
