...
Current state: backend production deploy `0.1.35-backend.50` is complete, `/opt/languagevoicetutor/backend/current` points to that release at last verification, and `0.1.35-backend.49` is the previous rollback reference. Production migration `20260620165657_AddAdminRoleAssignmentPersistence` has been applied. Production now has the Admin RBAC persistence tables `admin_users`, `admin_user_roles`, and `admin_role_assignment_events`.
