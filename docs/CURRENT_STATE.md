...
Desktop backend profile state: Debug/local development builds default to `http://localhost:5000`, while Release tester builds and Inno tester/release packages must use `https://api.languagevoicetutor.com`. Empty saved Backend URL settings use the current build default. In production tester/release mode, unsafe stale local overrides such as localhost, 127.0.0.1, or plain-http development URLs are ignored so they cannot silently replace the packaged production API. Backend connectivity is verified separately with the health endpoint (`/health`, with `/api/health` fallback); optional cloud settings and account/subscription status endpoints are not used as the connectivity signal. Missing optional cloud settings/account status endpoints must fall back to local settings/default status and must not block auth or lesson start. Settings/Diagnostics show the effective Backend URL, backend health result, last backend/account/settings/auth error category, and last safe HTTP status for tester support. Clean-machine smoke must verify backend health, direct production auth/register, in-app registration, login, lesson start/completion, progress/history, password reset, update check, and local settings fallback when optional cloud settings/account status are unavailable from a real installed build. Tester handoff is blocked until the installed-build auth/settings fallback issue is verified fixed on the problematic Windows device and a second Windows device. The backend remains the source of truth, the desktop still does not contain OpenAI keys or call OpenAI directly, production billing remains deferred, and public release/external tester handoff remain blocked until server-connected CMS/Admin verification, manual update check verification, clean-machine install, and the controlled tester checklist pass.
