...
Production manual smoke initially showed 4 new reports. Opening Feedback & reports left the badge at 4; changing one test App problem report from New to Reviewed reduced it immediately to 3 without a page reload. Overview **Available workflows** is now a native collapsed-by-default details control and remains expandable. System **Capabilities Check** and **Release / Capability Status** are visually hidden while their DOM elements and capability-processing logic remain intact. System **AI Models** remains visible and functional.

Pre-deploy verification confirmed clean `HEAD` and `origin/main` at `4da1e9147795d80f713a63ea815acfa5ca03e10a`, the backend Linux deployment policy passed, the combined `AdminFeedbackReportsUiStaticTests` and `AdminCapabilitiesServiceTests` run passed 23 tests with zero failures or skips, real `admin.js` syntax parsing passed, `git diff --check` passed, and the Release linux-x64 package built successfully. Production verification confirmed `current` at `.158`, `previous` at `.157`, `languagevoicetutor-backend.service` active/running, `/health` HTTP 200 `Healthy`, and `/api/health/database` HTTP 200 `Healthy` with `canConnect=true`. Deployed `index.html`, `admin.js`, and `admin.css` checks confirmed the expected `.158` Admin UI content; Admin CMS login/dashboard/logout/re-login, workflow collapse, badge behavior, hidden System diagnostics, and unchanged AI Models all passed manual smoke. Recent service logs showed normal `.158` startup with content root `/opt/languagevoicetutor/backend/releases/0.1.35-backend.158` and successful Admin login. No rollback was required.

Production AI Models remain unchanged: Lesson Tutor Chat, Feedback / correction, Lesson Hint, and Translation all use `gpt-5.6-luna`, and all four **Omit temperature parameter** flags remain enabled. `.158` introduced no EF migration or database schema/data change, new feedback endpoint, unread/read persistence, Admin RBAC or permission change, account-deletion workflow change, authentication contract change, billing/Premium/Paddle/Google Play/RTDN/reconciliation change, Mobile or Desktop application release, Website CMS published-content change, AI model configuration change, or production secret/configuration change.

## Historical 2026-09-13 multilingual website SEO, sitemap, and backend `.157` production checkpoint

Production backend `0.1.35-backend.157` was current at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.157`; `.156` was retained as rollback. The service was active, and public `/health` and `/api/health/database` returned HTTP 200. `.157` contains the Website CMS sitemap-generator update that adds the six language-practice URLs; it required no EF migration, schema change, or data change. Source history includes `1f2c215b82df031f9e20f9f230668fcd8400ada6` (`Add language practice pages to sitemap`) and the final website correction `6512425e0765c96380aecd30d26d9656a849707a` (`Remove obsolete English CTA visuals`). The unrelated static HTML/CSS work does not require backend execution; this release specifically enables Website CMS Publish to generate the completed sitemap.
...
Pre-deploy verification passed with 19 focused Hint tests, zero failures or skips; the backend Release build completed with zero warnings and errors; and `git diff --check` passed. Android `0.1.0+9` / versionCode 9 was then used against production `.156`; manual checks on several non-English study languages confirmed that repeated Hint requests remained in the selected study language. The production multilingual Hint regression is considered fixed.

Admin CMS AI Models were verified after deployment: Lesson Tutor Chat, Feedback / correction, Lesson Hint, and Translation all remain `gpt-5.6-luna`, with all four **Omit temperature parameter** flags enabled. No AI Models setting was edited or published as part of `.156`. The release changed no authentication, authorization, subscription, Google Play Billing, Paddle, RTDN, reconciliation, database schema/data, CMS published content, Mobile code, normal Lesson Chat behavior, Feedback, Realtime / Conversation mode, model selection, temperature setting, or production configuration.

Android `0.1.0+9` / versionCode 9 is publicly available in Google Play Production. Its source version bump is commit `0af802958a6a42116ecc1d8084ebdecc12e11406`, and it includes the Free Conversation regression correction from `fefbac3f366920b980f831ddd55c18e2734471a0`. The dated v8 publication and Restore Credentials evidence below remains historical and is not relabeled as v9 validation.

## 2026-09-04 consent-aware Google Analytics and backend `.155` production checkpoint

At this 2026-09-04 checkpoint, production backend `0.1.35-backend.155` was current at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.155`, with `.154` retained for rollback. Accepted source commit `3821c02f284c291d47815157b97f906f4b4956d9` was deployed through the standard package, reviewed dry-run, and upload flow without an EF migration. `languagevoicetutor-backend.service` was active, and public `/health` plus `/api/health/database` returned HTTP 200.
...
The current Active text configuration is `gpt-5.6-luna` for Lesson Tutor Chat, Feedback / correction, Lesson Hint, and Translation, with all four omit-temperature flags enabled. Runtime mapping is Lesson Tutor Chat -> `LessonTutorChatModel`, Feedback / correction -> `FeedbackCorrectionModel`, Lesson Hint -> `LessonHintModel`, Translation -> `TranslationModel`, and Lesson Summary -> `LessonTutorChatModel` intentionally. Each affected path evaluates temperature against the selected role model and records/logs the same model sent to the provider. The compatibility `OpenAiOptions.Model` property remains mapped to `LessonTutorChatModel` for existing non-target consumers.

Production validation boundaries are explicit. After `.153`, a real lesson worked, Lesson Summary was produced successfully, and a bounded journal query found no recurrence of the prior summary failure messages. After `.154`, the operator confirmed lesson start/flow, Lesson Hint, and Translation in production. Because all four Active text roles currently use the same `gpt-5.6-luna` ID, independent role routing is established by the 37 passing request-shape/policy tests with deliberately different fake model IDs and matching usage telemetry, not by claiming four visibly different production model IDs. No separate new `.154` Lesson Summary smoke is claimed.

A bounded request-count investigation found five `lesson_chat_reply` provider-response usage entries, four audio-transcription entries, two normal bounded Lesson Chat retries, no Lesson Chat provider failures, and no Lesson Summary failures. The retries were caused by first responses missing `feedback.cultureTip` on turns 2 and 3 and are expected under the existing maximum-two-attempt structured-response policy, not evidence of unconditional duplicate requests. Voice and Admin provider-access flows can legitimately generate additional distinct provider requests; no exact per-lesson total is claimed without a bounded session measurement.

## 2026-09-03 Production/public Android, backend, and website closeout

Android `0.1.0+8` / Google Play versionCode `8` is publicly available as **Orralen - Language Voice Tutor**. The existing v8 Play artifact was selected as the Production candidate, submitted for review after explicit owner approval, and public install availability was manually confirmed on 2026-09-03. This records promotion of the existing artifact, not a newly rebuilt or uploaded AAB. The public developer identity is **ORRALEN TECHNOLOGIES LTD**; the package remains `com.languagevoicetutor.mobile` and no account, entitlement, package, or billing-product migration occurred.

At this 2026-09-03 checkpoint, production backend `0.1.35-backend.151` was deployed at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.151`, with `.150` retained for rollback. It was deployed from the `f5fb15cb` static-homepage/CMS-ownership source line with no EF migration. `/health` and `/api/health/database` returned HTTP 200; Google Play Billing, RTDN, and reconciliation remained enabled. The Android Publisher credential, persistent Data Protection key ring, and active certificate remained outside release directories. An unauthenticated POST to the Google Play RTDN endpoint returned HTTP 401, confirming that the endpoint remained present and protected; checked public legal/support URLs returned HTTP 200.

The independent production files are `/index.html`, `/mobile.html` (a `noindex,follow` redirect to `/`), and `/styles.css`; Website CMS Publish must preserve them. CMS owns only `download.html`, `pricing.html`, `support.html`, `terms.html`, `privacy.html`, `refunds.html`, `cancellation.html`, `seller.html`, `ai-data.html`, `status.html`, `robots.txt`, `sitemap.xml`, optional `llms.txt`, and `marketing-consent.js`. The final publication deployed `.151` first, backed up the site at `/var/backups/languagevoicetutor/site/static-home-20260903_173550`, installed those independent files explicitly, then ran CMS Publish once. The post-publish SHA-256 values were `index.html` `c7e437258c292a41ff54e3d4e407002434ca5aa115d9b1a211d00062306d3a89`, `mobile.html` `b81b14e9e80a86fc257b7d51de3fdecd0656191761586fc256eab4aedc8cede7`, and `styles.css` `36e2dbfa38b1b0abe269a99321aeed9f0c8ffb6dc728ecab76fcda5f7b038895`. Public root, CMS pages/resources, and health checks returned HTTP 200; sitemap contains the root exactly once and excludes `/mobile.html` and `/ai-language-tutor/`; CMS Home navigation points to `/`; retired Mobile navigation is absent; and seller data retains ORRALEN TECHNOLOGIES LTD and 205 Regent Street.

The real-money first purchase and `.148` initial-deferral precision validation remain completed release evidence. Actual normal renewal scheduled for 2026-10-08, pending payment, fresh-install billing restore, refunds/voids, and chargebacks remain post-release observation work; neither Restore Credentials nor public availability proves those lifecycle paths.

## 2026-09-02 backend `.148` Google Play initial-deferral provider-precision validation PASS

Production backend `0.1.35-backend.148` is deployed through the normal repository release flow from source correction commit `ac4c5569162de8ff11a8ade2ee154802da41e28a` (`Handle Google Play expiry precision`). The current release is `/opt/languagevoicetutor/backend/releases/0.1.35-backend.148`; `.147` is retained as the previous rollback release. The service is active/running with `.148` as its content root, and public `/health` returned HTTP 200 on three consecutive checks while database health returned HTTP 200. No EF migration, Mobile/AAB change, Play Console configuration change, existing `.146`/`.147` ambiguous-terminal repair, or new provider mutation outside the controlled claim was performed.

A fresh controlled license-test claim verified successfully: the subscription was active with `CancelAtPeriodEnd=false`, its plan completed with `AttemptCount=1` and no error, acknowledgement was no longer pending, reconciliation count was zero with no next provider check, and global acknowledgement and overdue checks were zero. Logs recorded `target_converged`; RTDN was processed with no new permanent failure or provider rejection. A temporary PostgreSQL serializable conflict during purchase persistence recovered through the existing bounded retry and is not an open blocker.

The immutable local target was `...06.283460 CEST`; both the defer response and fresh authoritative provider state were `...06.283000 CEST`. The persisted subscription end is the authoritative `.283000` value (a -460 microsecond delta). This exercised the Google-only absolute UTC target-equivalence rule strictly below one millisecond: the planned target was preserved, actual provider expiry was persisted, no arbitrary intermediate snapshot became authoritative, strict fail-closed behavior remained for larger/non-equivalent differences, and no second defer was issued. This closes only the Google initial Premium-deferral provider-precision blocker; it does not establish public Play Production release/final approval, pending-refund handling, broader lifecycle evidence, October renewal, a new Mobile artifact, or a migration.

## 2026-09-02 backend `.147` Google Play provider-precision validation blocker

Production backend `0.1.35-backend.147` is deployed and healthy. A fresh explicitly allowed license-test purchase verified successfully, was acknowledged, and issued exactly one defer mutation. `.147` correctly classified the first post-defer GET as `intermediate_expiry_not_yet_converged`, but the plan still became `ambiguous_terminal` instead of converging.

Database evidence proved the remaining cause: the locally calculated immutable target was internally consistent, while both the `DeferAsync` response and the subsequent authoritative provider state agreed on an expiry exactly 460 microseconds earlier. Strict `DateTimeOffset` equality therefore rejected Google’s provider-normalized timestamp even though response and authoritative GET matched each other. The newer `.148` checkpoint records the reviewed correction, deployment, and controlled validation; this `.147` incident record remains historical.

## 2026-09-02 backend `.146` Google Play initial-deferral validation blocker

Production backend `0.1.35-backend.146` is deployed and healthy, but its controlled production validation did **not** close the Google Play initial Premium deferral blocker. A fresh explicitly allowed license-test purchase was verified successfully and acknowledged. Its one defer mutation returned the intended target at provider precision, yet the plan immediately became `ambiguous_terminal` with `trial_deferral_provider_state_diverged` and no authoritative provider expiry. A normal subscriptions-v2 verification only seconds later persisted the same provider expiry. The same purchase then produced RTDN rows ending as `permanent_failure` and `provider_rejected` even though the user had not canceled.

This independently reproduces the same post-defer convergence failure class previously seen with a real-money purchase. The proven evidence is that the mutation response already carried the expected target at provider precision and a later authoritative GET converged to that same provider value; the contents of the first post-defer snapshot were not captured sufficiently to prove that expiry was its only difference. At this `.146` checkpoint, no `.147` deployment, production-row repair, database migration, Mobile release, Play Console change, configuration change, or further provider mutation was claimed. The newer `.147` checkpoint above records the subsequent validation and exact provider-precision evidence.

## 2026-09-01 Google Play purchase-gate legacy-data checkpoint

Production backend remains `0.1.35-backend.145`; this was an operational data investigation and repair, not a backend release. Mobile remains the Play Internal-testing `0.1.0+8` / versionCode 8 candidate. The investigation established that `googlePlayPurchaseAllowed=false` correctly hid Mobile's **Get Premium** action for affected accounts: ten legacy pre-Live local Paddle Premium rows still recorded `active` after their locally known July 2026 end dates. Read-only checks found no later webhook for those rows and the eight untested remaining subscription IDs were not resolvable from the currently configured Paddle Live subscriptions API. That supports an operational legacy pre-Live interpretation; it does not prove that the records were Paddle Sandbox objects.

After a fresh production backup, a controlled two-row test and then a guarded transaction repaired the remaining eight stale rows: six became `expired` and two locally scheduled cancellations became `canceled`. No rows, payment history, provider IDs, webhook/event history, or entitlement architecture were deleted or rewritten; the transaction committed and `RemainingStaleActivePaddle=0`. Affected accounts then received Google Play purchase eligibility and displayed **Get Premium** as expected.

This one-time repair did not loosen `SubscriptionStatus`'s fail-closed Google Play new-purchase gate. A live external subscription with uncertain or ambiguous renewal ownership continues to block a Google Play launch to protect against double billing; only the pre-existing evidence rules determine that decision at runtime. Google Play License testing is isolated to the dedicated `pay` list with one intended tester rather than the broad Internal Testers list; non-license testers see normal payment methods. The prior controlled license-test purchase, renewal, reconciliation, and expiry evidence remains valid.

On 2026-09-01, a non-license-test account completed the first real-money Google Play Premium purchase from the Play-installed v8 build using normal payment methods. The normal receipt completed, Admin CMS and Mobile both showed backend-confirmed active Google Play Premium, and provider-management state showed active auto-renew with next payment on 2026-10-08. The account's existing backend registration trial and continuous Premium coverage were preserved: the provider-backed Premium tail reached 2026-10-08 through the existing backend-owned initial Premium/trial deferral mechanism. Google Play still has no Play trial or introductory offer; the registration trial is backend-owned. Any purchase-time receipt baseline date is not asserted as the final schedule, because the fresh post-deferral provider-management state is authoritative.

This is a PASS for the real-money first-purchase path and the initial Premium/trial deferral/provider schedule. An actual real-money renewal charge on 2026-10-08 has not yet occurred and remains unverified, as do pending-payment, explicit pre-expiry cancellation, fresh-install billing restore, refund/voided-purchase, chargeback, and public-rollout lifecycles. No code, migration, package, configuration change, restart/deployment, Mobile build/AAB, Play release, or Paddle provider mutation occurred.
...
No Mobile release, Website CMS publish, static website deployment, billing/Paddle/Google Play enablement, authentication change, or entitlement-architecture change occurred. At that historical checkpoint, Google Play was disabled. Language Voice Tutor remains the product/application name, the stable Windows AppId remains `LanguageVoiceTutor.Desktop`, and the existing ORRALEN icon/shortcut behavior remains unchanged. Code signing remains deferred. This is a public Windows direct release and verified backend deployment, not a full broad production-readiness claim.
...
This was a Windows direct-release upload only. No backend deployment, EF migration, database change, Mobile release, Google Play enablement, Website CMS publish, or static website publish occurred. Production backend at that historical checkpoint was `0.1.35-backend.140` with `0.1.35-backend.139` retained as rollback; Google Play was disabled at that time.
...
This was a Windows static-release upload only: no backend deployment or migration occurred. At that historical checkpoint production was `0.1.35-backend.134` with `.133` as rollback; Google Play was disabled. Paddle, subscriptions, Website CMS publication, lesson scenarios, and backend lesson-context behavior were not changed. Generated `artifacts/` outputs remain uncommitted, code signing remains deferred, and this public Windows Direct release is not a broad production-readiness claim. Later production state is documented at the top of this file.
...
Main contains the accepted disabled-by-default Google Play purchase-verification foundation through commit `335ef8a0`: authenticated token verification, SHA-256 purchase-token ownership fingerprinting, a sanitized subscriptions-v2 verifier, lazy ADC-backed wiring, and internal verified ProductId/UTC period/acknowledgement/test-purchase metadata. The initial foundation was dormant; later repository changes connected `GooglePlayPurchaseVerificationService` to atomic verified-purchase persistence, which uses provider-scoped verified-period persistence internally to maintain the exact `google_play` Subscription and linked Premium entitlement. The public endpoint is `POST /api/me/billing/google-play/purchases/verify`; in historical deployed `.134` infrastructure it returned `503 not_configured` because Google Play was disabled at that time. Migration `20260727045935_AddGooglePlayPurchaseClaims` is applied in production.

The authenticated Google Play verification service passes only trusted verified metadata into atomic ownership/Subscription/entitlement persistence. This disabled infrastructure was deployed in historical `.134`, and migration `20260727045935_AddGooglePlayPurchaseClaims` is applied; at that checkpoint Google Play was disabled, so actual purchase processing had not been enabled or tested and the production claim table was empty. Test purchases were unsupported, no acknowledgement occurred, and no Payment or BillingEvent projection was created. Repository-only account-status multi-provider selection remains entitlement-driven and protects valid Paddle Premium; Paddle checkout, webhook processing, Premium activation/cancellation/refund behavior, and Desktop Premium-expiry display remain unchanged. At that checkpoint, later enablement and production rollout were still pending.
...
The home page shows the logo, supported study language flags, a Windows desktop app card, and accurate mobile wording. Android `0.1.0+10` / versionCode 10 is publicly active in Google Play Production; do not claim iOS availability and do not say “Mobile version coming soon”.
...
Historical persistence verification: `/opt/languagevoicetutor/backend/site/content/ai-model-settings.json` was seeded from the then-current release only because the persistent file was missing, had mode `644`, survived a `languagevoicetutor-backend.service` restart, and matched that release copy by SHA-256 (`94f84fc07551d821bfa9dc0682bb4ee60108d11d74987b84ebb39fce96f825f1`). At that checkpoint it contained `gpt-5.5` plus `gpt-5.2`; the current Active model values are recorded below. This resolved the AI Models persistence risk. Future backend deploys must not rely on `/opt/languagevoicetutor/backend/current/site/content/ai-model-settings.json` or any `/opt/languagevoicetutor/backend/releases/<version>/site/content/ai-model-settings.json` file as the production source of truth.

Current known-good AI model configuration: lesson tutor chat, feedback/correction, lesson hints, and translation each use `gpt-5.6-luna`, and each text role has **Omit temperature parameter** enabled. Speech-to-text uses `gpt-4o-mini-transcribe`; normal lesson chat TTS uses `tts-1`; Conversation Mode TTS uses `gpt-4o-mini-tts`; and Realtime voice uses `gpt-realtime`. These are model IDs and non-secret compatibility flags only; they do not include provider credentials.

Current Active production values are `gpt-5.6-luna` for lesson tutor chat, feedback/correction, lesson hint, and translation, with all four omit-temperature flags enabled. Publishing AI model settings affects new backend AI requests without a desktop release because the desktop continues to call backend endpoints and does not choose OpenAI model IDs. The Super Admin workflow is: Load AI Models → Edit draft → Save draft → Validate format → Test provider access → Review compatibility diagnostics → Publish / Make active only if relevant runtime diagnostics pass → run a small real lesson after publishing. Validate format checks syntax only and does not prove provider access. Test provider access performs provider-level checks using draft settings, does not publish settings, and uses safe dummy input rather than real lesson/user text. Audio and realtime roles may be `not_tested` when not covered by lightweight provider tests.

The earlier `gpt-5.5` lesson tutor chat investigation found that `gpt-5.5` was available to the deployed OpenAI API key/project. The root cause was the request parameter `temperature`, not model unavailability. Safe provider diagnostics recorded `statusCode: 400`, `safeCategory: invalid_request`, `providerErrorType: invalid_request_error`, `providerErrorParam: temperature`, and `sanitizedProviderMessage: Unsupported parameter: 'temperature' is not supported with this model.` Minimal Responses API text, minimal structured output, and the lesson runtime shape without user content passed after `temperature` was omitted. This remains compatibility evidence; it is not the current Active model configuration.
...
Deployed production behavior: `.152` added four independent Draft/Active omit-temperature overrides for lesson tutor chat, feedback/correction, lesson hint, and translation; `.153` made Lesson Summary use the Lesson Tutor Chat effective-temperature policy; and `.154` routes Lesson Tutor Chat, Feedback / correction, Lesson Hint, and Translation through their corresponding independent model IDs while Summary intentionally remains on `LessonTutorChatModel`. A missing or `false` flag preserves that role's legacy request shape; `true` forces the `temperature` property to be absent. Save draft, format validation, Draft provider testing, reset from Active, and publish preserve all four flags. Draft provider tests and runtime requests apply temperature policy to the exact selected role model. Speech-to-text, both text-to-speech roles, and Realtime voice have no temperature flag and are unaffected.

Compatibility diagnostics are interpreted as follows: `minimal_responses_text` verifies basic model availability and Responses API access without `temperature`; `current_provider_test_shape` verifies the lesson tutor role's current effective minimal provider-test shape, including `temperature` only when its legacy policy and Draft override resolve to a numeric value; `minimal_structured_output` verifies strict structured output support using a tiny safe schema without `temperature`; and `lesson_chat_runtime_shape_without_user_content` verifies lesson runtime request options/schema with safe dummy input and the same effective-temperature policy. If the minimal text check fails, suspect project/key availability or alias usage. If minimal text passes but the current provider-test shape fails, inspect the added parameter. If structured output fails, schema/text-format compatibility is the issue. If structured output passes but lesson runtime shape fails, the lesson schema or runtime request shape is the issue. If the lesson runtime shape passes, the model is safe to try in a small real lesson.
