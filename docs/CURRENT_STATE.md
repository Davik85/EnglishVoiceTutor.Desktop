...
Current state: verified production backend snapshot is `/opt/languagevoicetutor/backend/releases/0.1.35-backend.52`, and `/opt/languagevoicetutor/backend/current` points to that release. Verify the live value with the server symlink command before calling it current.
...
Current state: backend production deploy `0.1.35-backend.52` is complete, `/opt/languagevoicetutor/backend/current` points to that release at last verification. Production migration `20260620165657_AddAdminRoleAssignmentPersistence` has been applied. Production now has the Admin RBAC persistence tables `admin_users`, `admin_user_roles`, and `admin_role_assignment_events`.
...
- admin roles/permissions policy and UI policy tests passed, the desktop release gate passed, and backend `0.1.35-backend.52` is deployed and healthy with Admin RBAC persistence migration applied.
