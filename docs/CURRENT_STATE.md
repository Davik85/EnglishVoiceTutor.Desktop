...
## Website CMS initialization current state (2026-06-26)

Website is now a top-level Admin Shell tab, not a `CMS Content` sub-tab. The tab loads an admin-only read-only metadata overview for expected Website CMS sections and can initialize missing `website_cms_sections` rows with empty metadata only. It does not expose body content, add legal text editing, save draft bodies, publish content, change public website rendering, or modify `site/public/`. Content editing and publish remain deferred. `CMS Content` remains focused on learner/runtime content packs.

The Website CMS backend foundation exists through the `website_cms_sections` table and migration `20260625090000_AddWebsiteCmsLegalContentFoundation`. Production DB rollout was applied manually from reviewed SQL; production `__EFMigrationsHistory` contains the migration, and indexes `IX_website_cms_sections_ReviewStatus` and `IX_website_cms_sections_SectionKey` plus `PK_website_cms_sections` exist. The Website CMS metadata endpoint is live, the Admin Website initialization action was run in production, and `website_cms_sections` privileges for runtime role `lvt_app` were corrected. Production DB verification confirmed 9 rows exist for `seller_company`, `support`, `pricing`, `terms`, `privacy`, `refunds`, `cancellation`, `ai_data_disclosures`, and `platform_status`; all initialized rows have `ReviewStatus=not_started`, `DraftBody` length `0`, and `PublishedBody` null / `has_published=false`.

The static public website remains the actual public rendering source. Production public website root is `/var/www/languagevoicetutor/site`; the accidental parent-directory upload was quarantined at `/var/www/languagevoicetutor/_mistaken_static_upload_20260625`. Public Website CMS rendering is not connected, no draft content may be served publicly, and live Paddle is not enabled. Final legal/seller/support/pricing values still need owner/legal approval.

## Current production backend state

Current state: verified production backend snapshot is `/opt/languagevoicetutor/backend/releases/0.1.35-backend.54`, and `/opt/languagevoicetutor/backend/current` points to that release. Verify the live value with the server symlink command before calling it current.
...
Current state: backend production deploy `0.1.35-backend.54` is complete, `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.54` at last verification. Production migration `20260620165657_AddAdminRoleAssignmentPersistence` has been applied. Production now has the Admin RBAC persistence tables `admin_users`, `admin_user_roles`, and `admin_role_assignment_events`.
...
- admin roles/permissions policy and UI policy tests passed, the desktop release gate passed, and backend `0.1.35-backend.54` is deployed and healthy with Admin RBAC persistence migration applied.
...
There is still no published Website CMS content, no Website CMS public rendering, no public unauthenticated Website CMS route, no live Paddle enablement, and no change to static `site/public` rendering. Draft copy is not final legal advice or legal approval.
