...
- Do not change Paddle, billing, subscription, entitlement, lesson JSON, Study languages, Interface languages, or Native/Explanation language catalog without an explicit later task. Step 5D-1 added backend CMS schema foundation, Step 5D-2 added a development/admin-only CMS static JSON import foundation, and Step 5D-3 added a safe backend published-snapshot read/status path. Admin UI editing, learner-facing runtime CMS rollout, preview UI, rollback, and production CMS operations still require explicit future tasks.
