...
Current backend release after the learner `CurrentLevel` settings deployment: `/opt/languagevoicetutor/backend/releases/0.1.35-backend.116`. Previous backend release before this deployment was `/opt/languagevoicetutor/backend/releases/0.1.35-backend.115`; `0.1.35-backend.114` was already active before the `.115` voice scenario deployment and must not be described as containing the `.115` structured-output fix. Deployment completed successfully: `languagevoicetutor-backend.service` was active, public `/health` returned HTTP 200, and public `/api/health/database` returned HTTP 200 with `canConnect=true`. No EF migration or schema change was added or run, and Website and Windows installer/release files were unchanged. Current-state docs must not use the obsolete phrase “current backend release is `0.1.35-backend.99`” except when explicitly identifying it as outdated wording. Previous backend rollback reference should be verified from `/opt/languagevoicetutor/backend/previous`; the last documented rollback reference before this handoff was `0.1.35-backend.49`, but operators must verify the symlink before rollback. Older documentation-source policy baselines such as `0.1.35-backend.50` are not the current backend release for this handoff.

Backend deployment uses:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\package-backend-linux-release.ps1 -Version 0.1.35-backend.116
powershell -ExecutionPolicy Bypass -File .\scripts\upload-backend-linux-release.ps1 -Version 0.1.35-backend.116
```
...
## 2026-07-17 authenticated user-feedback reporting foundation

Backend `0.1.35-backend.117` is production-deployed and healthy for the authenticated feedback reporting foundation, with previous backend release `0.1.35-backend.116`. The live backend `current` symlink was switched to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.117`; `languagevoicetutor-backend.service` restarted successfully; public `/health` returned HTTP 200; public `/api/health/database` returned HTTP 200; and the backend upload script did not run a migration.

The backend now records authenticated user feedback through `POST /api/me/feedback-reports`. The `UserId` comes only from JWT claims, so Mobile cannot submit another user id. Supported categories are `suggestion`, `app_issue`, and `ai_response`. `Message` is required, trimmed, and limited to 4000 characters; `ReportedAiText` is optional and limited to 4000 characters; `ClientPlatform` is limited to 32 characters; `ClientVersion` is limited to 64 characters; stored status is always `new`; and `CreatedAtUtc` is generated by the backend. Persistence failures return a safe HTTP 503 without database exceptions, connection strings, stack traces, or internal paths. Success returns HTTP 201 with `{ "reportId": "...", "status": "received" }`.

Migration `20260717120148_AddUserFeedbackReports` added `user_feedback_reports` in the existing backend database after previous production migration `20260701000000_AddAdminAuthAuditEvents`. The table records `Id`, `UserId`, `Category`, `Message`, `ReportedAiText`, `Status`, `ClientPlatform`, `ClientVersion`, `CreatedAtUtc`, and `ReviewedAtUtc`; it has a foreign key to `users` and restricts deletion of a referenced user where defined by the migration. The reviewed production SQL was generated from the previous migration to the new migration and contained only `CREATE TABLE user_feedback_reports`, expected indexes, the foreign key, and the migration-history insert, with no `DROP TABLE`, `DROP COLUMN`, `DELETE`, or `TRUNCATE`. It was applied manually with `ON_ERROR_STOP` enabled, and migration history was verified.

Production initially exposed a permissions issue because the migration was applied as `postgres`, leaving `user_feedback_reports` owned by `postgres`. The app role `lvt_app` could connect but could not insert, causing HTTP 503 from `POST /api/me/feedback-reports` and PostgreSQL `42501` (`permission denied for table user_feedback_reports`) in backend logs. The table owner was changed to `lvt_app`, `SELECT`, `INSERT`, `UPDATE`, and `DELETE` were granted to `lvt_app`, the owner was confirmed as `lvt_app`, and `lvt_analytics_reader` was confirmed to have `SELECT`. No backend restart was required. After correction, suggestion, app problem, and AI response report submissions succeeded, storing three production smoke-test rows with status `new`, `ClientPlatform` `android`, and `ClientVersion` `0.1.0+1`.

Operationally, future reviewed SQL applied under `postgres` that creates application tables must verify the new objects' owner and grants before the migration is considered complete. The expected owner for `user_feedback_reports` is `lvt_app`; do not apply a blanket ownership change to unrelated existing tables.

Product boundary: there is still no CMS feedback-management screen, no email delivery, no attachments or screenshot upload, no automatic moderation, no OpenAI forwarding/API call, no separate feedback database, and no subscription, billing, lesson, TTS, transcription, or tutor-prompt behavior change. A future CMS screen for reviewing `new`, `reviewed`, and `resolved` reports remains separate work.
...
- Backend release in tracked release docs: current production is `0.1.35-backend.116`, previous is `0.1.35-backend.115`, and the live `/opt/languagevoicetutor/backend/current` symlink resolves to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.116`; `/health` and `/api/health/database` are verified healthy. Backend .99/.108/.112 references are historical and not current production unless a section is explicitly documenting those older releases.
