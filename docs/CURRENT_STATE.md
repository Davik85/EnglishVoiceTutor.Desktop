...
Desktop backend profile state: Debug/local development builds default to `http://localhost:5000`, while Release tester builds and Inno tester/release packages must use `https://api.languagevoicetutor.com`. Release builds ignore saved Backend URL settings completely, so stale localhost, 127.0.0.1, plain-http, local-network, or other user-editable AppData URLs cannot replace the packaged production API. Backend connectivity is verified separately with the health endpoint (`/health`, with `/api/health` fallback); optional cloud settings and account/subscription status endpoints are not used as the connectivity signal. Missing optional cloud settings/account status endpoints must fall back to local settings/default status and must not block auth or lesson start. Release Settings keeps the user-facing sections simple and does not include Diagnostics or Backend URL editing; safe internal file logs may still be used for support. Clean-machine smoke must verify backend health, direct production auth/register, in-app registration, login, lesson start/completion, progress/history, password reset, update check, and local settings fallback when optional cloud settings/account status are unavailable from a real installed build. Tester handoff is blocked until the installed-build auth/settings fallback issue is verified fixed on the problematic Windows device and a second Windows device. The backend remains the source of truth, the desktop still does not contain OpenAI keys or call OpenAI directly, production billing remains deferred, and public release/external tester handoff remain blocked until server-connected CMS/Admin verification, manual update check verification, clean-machine install, and the controlled tester checklist pass.
...
- Packaged Release hides Diagnostics by default.
- Diagnostics is not part of normal Release Settings; use safe internal logs for support/developer investigation.
- Do not add backend URL editing or diagnostics tabs back into user/release Settings.
...
## Desktop 0.1.26 release-blocker fixes: adaptive welcome layout and update-safe session persistence

- Welcome/start screen primary actions are visible on smaller laptop screens without scrolling. The welcome page no longer uses a root `ScrollViewer`; it clamps the card to the actual visible window area, reserves a primary action row, and shrinks/crops the hero image before it can push Start lesson or Settings below the fold.
- The app clamps startup size and position to the visible working area and adapts content instead of pushing actions below the fold. This keeps the title bar, close button, Settings access, and primary navigation usable on smaller laptop / scaled display targets such as 1366x768 at 125% and 1280x720 at 100%.
- Installer update/reinstall preserves auth session, settings, history, and progress. The Inno installer updates the application directory only and does not delete roaming or local AppData session files; reinstall/update should preserve user app data and session storage.
- Auth session storage remains server-only and DPAPI CurrentUser-protected. The stable current path is `%APPDATA%\EnglishVoiceTutor.Desktop\auth-session.json`; legacy `%APPDATA%`/`%LOCALAPPDATA%` LanguageVoiceTutor folder names are read once when present, validated, then re-saved to the stable path without storing plaintext tokens.
- Release Settings still hide Diagnostics and Backend URL controls. Backend/auth/lessons remain unchanged and installed tester/release builds remain locked to `https://api.languagevoicetutor.com`.
