...
Database evidence proves the remaining cause: the locally calculated immutable target was internally consistent, while both the `DeferAsync` response and the subsequent authoritative provider state agreed on an expiry exactly 460 microseconds earlier. Strict `DateTimeOffset` equality therefore rejected Google’s provider-normalized timestamp even though response and authoritative GET matched each other. The bounded source correction accepts only a UTC-instant difference strictly below one millisecond for Google trial-deferral target convergence and persists the actual authoritative Google expiry; a difference of one millisecond or more remains non-equivalent. Public Google Play rollout remains blocked until this correction is reviewed, deployed, and production-validated. No `.148` deployment, production-row recovery, database migration, configuration change, Play Console change, or further provider mutation is claimed.
