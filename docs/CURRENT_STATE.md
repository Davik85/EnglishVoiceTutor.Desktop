...
- Backend: production is deployed and healthy at `https://api.languagevoicetutor.com` on `0.1.35-backend.148`, with `.147` retained as rollback. The prior `.142` account-wide Premium/purchase-gating foundation and additive Google Play trial-deferral migration remain deployed; `.144` and `.145` remain historical Restore Credentials releases, while `.148` closes only the initial-deferral provider-precision validation blocker without an EF migration. Production Backend Data Protection remains enabled. Controlled Google Play Internal testing is enabled; public rollout remains pending separate gates.
...
The current backend release is `0.1.35-backend.148`, with `.147` retained as rollback. The deployed account-deletion flow includes migrations `20260722132656_AddAccountAnonymizationPreflightFoundation` and `20260723045852_AddAccountAnonymizationExecution`; Google Play foundation migrations and `20260831080122_AddRestoreCredentialsFoundation` are applied. Public backend and database health returned HTTP 200; `.142` applied the additive Google Play trial-deferral foundation migration, while `.144` through `.148` required no further migration. Backend Data Protection is enabled in production with its persistent key ring and active certificate outside release directories. Previous backend rollback reference must always be verified from `/opt/languagevoicetutor/backend/previous` before rollback.
...
Unreleased source behavior, not production-deployed: the AI Models Draft/Active schema has four independent omit-temperature overrides for lesson tutor chat, feedback/correction, lesson hint, and translation. A missing or `false` flag preserves that role's legacy request shape; `true` forces the `temperature` property to be absent. Save draft, format validation, Draft provider testing, reset from Active, and publish preserve all four flags. Draft provider tests apply the same effective-temperature policy as each current runtime request. Speech-to-text, both text-to-speech roles, and Realtime voice have no temperature flag and are unaffected.
...
- Backend release in tracked release docs: current production is `0.1.35-backend.148`, with `.147` as rollback; `/health` and `/api/health/database` are verified healthy. Older backend references are historical and not current production unless a section is explicitly documenting those releases.
