...
- Backend: production is deployed and healthy at `https://api.languagevoicetutor.com`; current backend release is `0.1.35-backend.94`.
...
5. Admin auth audit persistence is production-applied for verified login/logout events; `admin_login_failed`, `disabled_admin_login_denied`, and session expiration audit persistence remain pending until separately verified/implemented.
...
Current backend release: `/opt/languagevoicetutor/backend/releases/0.1.35-backend.94`. Previous backend rollback reference should be verified from `/opt/languagevoicetutor/backend/previous`; the last documented rollback reference before this handoff was `0.1.35-backend.49`, but operators must verify the symlink before rollback. Older documentation-source policy baselines such as `0.1.35-backend.50` are not the current backend release for this handoff.

Backend deployment uses:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\package-backend-linux-release.ps1 -Version 0.1.35-backend.94
powershell -ExecutionPolicy Bypass -File .\scripts\upload-backend-linux-release.ps1 -Version 0.1.35-backend.94
```
...
Production Admin RBAC / persistent role management is completed after backend release `0.1.35-backend.94`, deployed by the normal backend package/upload flow. The production backend `current` symlink was verified at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.94`; `/health` and `/api/health/database` returned `200 Healthy`. No EF migrations were added or run for this RBAC stage. Windows installer release files were not changed.
...
Admin Activity first production slice is completed: the Admin Activity tab is visible and usable, includes `admin_actions`, `admin_role_assignment_events`, and the production-applied `admin_auth_audit_events` source, displays admin-entered reasons/notes where stored, and keeps `safeMetadataJson` separate from Admin note. Production verification has shown `admin_login_success` and `admin_logout` rows in Admin Activity; `admin_login_failed`, `disabled_admin_login_denied`, session expiration audit persistence, and any Website/AI publish audit coverage not already represented in existing audit tables remain pending until separately verified/implemented.
...
The `0.1.36-tester.31` Windows direct tester release has been built, uploaded, and verified. The user confirmed the newly uploaded build works and that the manual-confirmation update flow works on other devices. Backend deployment was not part of this desktop polish release; production backend remains healthy at `0.1.35-backend.94`, and no database migrations were added or run.
...
- Backend release in tracked release docs: `0.1.35-backend.94`. The live `/opt/languagevoicetutor/backend/current` symlink was manually verified with `ssh lvt-server "readlink -f /opt/languagevoicetutor/backend/current"` and resolved to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.94`; `/health` and `/api/health/database` were also verified healthy. Backend .94 was deployed by the normal backend package/upload flow for the Admin Activity and emergency Premium revoke stage; this documentation task did not deploy backend code.
...
- Production Admin Activity includes the `admin_auth_audit_events` source dropdown entry and shows verified `admin_login_success` and `admin_logout` events. `admin_login_failed` is not documented as production-verified because the manual failed-login check was not included in the provided evidence; `disabled_admin_login_denied` remains pending.
...
Production backend current release is `0.1.35-backend.94`; the `current` symlink was verified at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.94`, `languagevoicetutor-backend.service` is active/running, `/health` returns `200 Healthy`, and `/api/health/database` returns `200 Healthy`. Backend .94 was deployed through the normal backend package/upload flow. The deploy script did not run EF migrations, and Windows installer files were not changed.
...
- Controlled Paddle live payment/webhook/Premium activation validation remains pending. Session expiration audit persistence remains pending; `admin_login_failed` and `disabled_admin_login_denied` remain pending until manual production checks are separately performed and documented.

## Admin auth audit persistence production verification (2026-07-01)

- Migration `20260701000000_AddAdminAuthAuditEvents` was applied before backend `0.1.35-backend.94` deployment, after fresh backup creation and SQL review.
...
- Production backend `0.1.35-backend.94` is deployed successfully; `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.94`, `languagevoicetutor-backend.service` is active/running, `/health` returns `200 Healthy`, and `/api/health/database` returns `200 Healthy`.
...
- Admin Activity includes `admin_auth_audit_events` as a read-only source, the source dropdown includes `admin_auth_audit_events`, and production Admin Activity shows verified `admin_login_success` and `admin_logout` events.
- `admin_login_failed` is not documented as production-verified because the provided evidence does not include the manual failed-login check. `disabled_admin_login_denied` remains pending.
- Session expiration persistence remains pending; no low-noise expiration persistence completion is claimed.
- Controlled Paddle live payment validation remains pending, including webhook/Premium activation, refund, chargeback, and customer portal checks.
