...
## Website CMS admin-only publish current state (2026-06-27)

Website is now a top-level Admin Shell tab, not a `CMS Content` sub-tab. The tab loads admin-only metadata, can initialize missing `website_cms_sections` rows, can select/load section detail, validate the stored draft, open an admin-only simple-text draft preview, update safe review statuses with required change reason, save admin-only draft fields, and explicitly publish to internal Website CMS `PublishedBody`. Publish requires non-empty `DraftBody`, non-empty `ChangeReason`, `legal_approved` status, and `WebsiteCmsContentGuard`; it copies `DraftBody` to `PublishedBody`, sets `PublishedAtUtc`, and updates metadata. `CMS Content` remains focused on learner/runtime content packs.
...
Backend `0.1.35-backend.57` is deployed, `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.57`, and health checks returned `200 Healthy` for `https://api.languagevoicetutor.com/health` and `https://api.languagevoicetutor.com/api/health/database`. Production smoke on `platform_status` saved a temporary draft, set review status to `legal_approved`, validated and previewed the draft, published it into internal `PublishedBody`, and confirmed `PublishedAtUtc` was set while public rendering did not change. Smoke data was manually cleaned up before the simple admin-only unpublish foundation existed. After cleanup, DB verification confirmed 9 rows, `ReviewStatus=not_started` for all rows, `DraftBody` length `0` for all rows, and `PublishedBody` null / `has_published=false` for all rows. Fresh service logs showed no new `website-cms` errors, permission-denied errors, exceptions, failures, or `500` responses.

The static public website remains the actual public rendering source. Production public website root is `/var/www/languagevoicetutor/site`; the accidental parent-directory upload was quarantined at `/var/www/languagevoicetutor/_mistaken_static_upload_20260625`. Saving, validating, previewing, changing Website CMS review status, or publishing internally does not update public website rendering, does not modify `site/public/`, and does not imply final legal approval. Admin-only publish can copy an approved non-empty DraftBody into internal `PublishedBody` with `PublishedAtUtc`, but no draft or published Website CMS content is served publicly, public rendering integration is not implemented, revision-history rollback is not implemented, simple unpublish only clears internal PublishedBody, and live Paddle is not enabled. Final legal/seller/support/pricing values still need owner/legal approval.
...
Phase 5C production logging hardening was first deployed on backend `0.1.35-backend.40`, was production-verified on backend `0.1.35-backend.50`, and is retained in current backend `0.1.35-backend.57`. The 2026-06-24 Phase 5C verification returned `200 OK` for health/database health and 0 lines for the bounded sensitive/EF SQL grep set: `Microsoft.EntityFrameworkCore.Database.Command`, `SELECT`, `INSERT`, `UPDATE`, `PasswordHash`, `TokenHash`, `RawPayload`, and `SignatureHeader`. The later Website CMS rollouts added the Website CMS foundation and admin-only publish workflow and included a separately applied Website CMS migration; production/live Paddle readiness remains deferred, and broad public production readiness is still not claimed.
...
Website CMS can now manage the admin-only internal lifecycle for initialized sections. The Admin Website tab supports metadata overview, missing-section initialization, section detail, draft-body save, stored-draft validation, admin-only safe-text preview, internal review-status changes with required `ChangeReason`, and explicit admin-only publish to Website CMS `PublishedBody`. Publish requires non-empty `DraftBody`, non-empty `ChangeReason`, `legal_approved` status, and `WebsiteCmsContentGuard`; it copies `DraftBody` to `PublishedBody`, sets `PublishedAtUtc`, and updates metadata.

Backend `0.1.35-backend.57` is deployed at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.57`; health and database health returned `200 Healthy`. Production smoke on `platform_status` saved a draft, changed review status to `legal_approved`, validated and previewed the draft, published it into internal `PublishedBody`, and confirmed `PublishedAtUtc` was set while public rendering did not change. Smoke data was manually cleaned up before the simple admin-only unpublish foundation existed. Post-cleanup DB verification confirmed 9 rows, all `ReviewStatus=not_started`, all `DraftBody` lengths `0`, and `PublishedBody` null / `has_published=false` for all rows. Fresh service logs showed no website-cms errors, permission denied errors, exceptions, failures, or `500` responses.

Website CMS now has an admin-only explicit publish workflow into internal `PublishedBody` storage, but there is still no Website CMS public rendering, no public unauthenticated Website CMS route, and no live Paddle enablement. Public rendering remains static `site/public`; public rendering integration is not implemented. Website CMS can unpublish internal PublishedBody only; revision-history rollback remains deferred. Final legal/seller/support/pricing copy still requires owner/legal approval.
