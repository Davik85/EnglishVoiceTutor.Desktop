...
Desktop and Mobile have not yet switched to `localizedSetup`. The next rollout order is: (1) review and commit; (2) push; (3) create and deploy a new backend release; (4) verify backend health; (5) import canonical content into the CMS draft; (6) validate and publish the CMS snapshot; (7) verify production `localizedSetup` responses; (8) switch Desktop; and (9) switch Mobile separately.

The Admin CMS scenario editor now exposes five full localized first-message textareas for French, German, Portuguese, Spanish, and Italian directly below the existing canonical English field. Each edits one complete `setupMessageTemplate` text block; messages are not split into Goal, situation, or instruction controls. Structured Save draft uses those visible fields, while Advanced JSON Save draft preserves the entered `DefinitionJson` as authoritative; the normal successful-save reload synchronizes visible fields from saved JSON. `contextVariantTitles` stay internal to Advanced JSON, draft save remains permissive, publication validation remains unchanged, and deployment/CMS publication remain pending. Desktop and Mobile remain unchanged.
