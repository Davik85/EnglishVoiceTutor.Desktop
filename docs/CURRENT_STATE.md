...
The production-safe script path is `/opt/languagevoicetutor-ops/postgres/backup_lvt_postgres.sh` because the `postgres` account must traverse the backup script path without gaining access to the backend release/config tree. Linux shell/systemd assets must use LF line endings, enforced through `.gitattributes`, because CRLF copied from Windows broke Bash on Ubuntu before correction. The service uses `WorkingDirectory=/tmp` to avoid `postgres` working-directory warnings during manual or systemd execution. Off-server encrypted backups, permission-fidelity restore drill, and migration rollback/remediation rehearsal remain future work; broad public production readiness and production/live Paddle readiness remain deferred.
