...
# Backend-owned localized lesson setup foundation

The lesson-content runtime now supports optional authored `setupLocalizations` in CMS scenario definition JSON and exposes an additive request-specific `localizedSetup` projection. The authenticated learner's backend setting selects the study language. English remains canonical; absent non-English authored content is explicitly `incomplete` without an English fallback. Desktop and Mobile have not switched to this projection, and localization authoring plus all-language publication enforcement remain future work. No migration, content publication, or deployment was performed.
