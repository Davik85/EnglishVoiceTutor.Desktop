...
A code-level operator-only recovery path is now required before production refund revocation can be considered verified. Until that recovery path is deployed and run, Paddle full-refund Premium revocation remains not production-verified. No additional live payment or refund testing should be performed. Manual Premium Revoke remains the operational stopgap if immediate protection is needed. Broad public paid launch remains pending. Customer portal remains pending unless separately verified. Direct installer code signing remains pending.
