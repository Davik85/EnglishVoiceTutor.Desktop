...
## Website CMS draft-save current state (2026-06-27)

Website is now a top-level Admin Shell tab, not a `CMS Content` sub-tab. The tab loads admin-only metadata, can initialize missing `website_cms_sections` rows, can select/load section detail, validate the stored draft, open an admin-only simple-text draft preview, update safe non-published review statuses with required change reason, and can save admin-only draft fields: `DraftBody`, `InternalNotes`, `EffectiveDate`, `ReviewStatus`, and required `ChangeReason`. Saves use `WebsiteCmsContentGuard` before persistence to block obvious secret-like/private/provider identifiers. `CMS Content` remains focused on learner/runtime content packs.
...
Backend `0.1.35-backend.55` is deployed, `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.55`, and health checks returned `200 Healthy` for `https://api.languagevoicetutor.com/health` and `https://api.languagevoicetutor.com/api/health/database`. Production smoke on `platform_status` saved a temporary draft, observed `ReviewStatus=draft` and `Draft exists = Yes`, then cleared the draft and restored `ReviewStatus=not_started`. After cleanup, DB verification confirmed 9 rows, `ReviewStatus=not_started` for all rows, `DraftBody` length `0` for all rows, and `PublishedBody` null / `has_published=false` for all rows. Fresh service logs showed no new `website-cms` errors, permission-denied errors, exceptions, failures, or `500` responses.
...
Current state: verified production backend snapshot is `/opt/languagevoicetutor/backend/releases/0.1.35-backend.55`, and `/opt/languagevoicetutor/backend/current` points to that release. Verify the live value with the server symlink command before calling it current.
...
Current state: backend production deploy `0.1.35-backend.55` is complete, `/opt/languagevoicetutor/backend/current` points to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.55` at last verification. Production migration `20260620165657_AddAdminRoleAssignmentPersistence` has been applied. Production now has the Admin RBAC persistence tables `admin_users`, `admin_user_roles`, and `admin_role_assignment_events`.
...
- admin roles/permissions policy and UI policy tests passed, the desktop release gate passed, and backend `0.1.35-backend.55` is deployed and healthy with Admin RBAC persistence migration applied.
...
Phase 5C production logging hardening was first deployed on backend `0.1.35-backend.40`, was production-verified on backend `0.1.35-backend.50`, and is retained in current backend `0.1.35-backend.55`. The 2026-06-24 Phase 5C verification returned `200 OK` for health/database health and 0 lines for the bounded sensitive/EF SQL grep set: `Microsoft.EntityFrameworkCore.Database.Command`, `SELECT`, `INSERT`, `UPDATE`, `PasswordHash`, `TokenHash`, `RawPayload`, and `SignatureHeader`. The later `0.1.35-backend.55` rollout added the Website CMS foundation and included a separately applied Website CMS migration; production/live Paddle readiness remains deferred, and broad public production readiness is still not claimed.

## Admin payment-event statistics

Completed and production-verified on backend `0.1.35-backend.55`: Admin Product Statistics now distinguishes access-state metrics from payment-event metrics. Live server verification of `wwwroot/admin/admin.js` showed `successfulPaymentsTotal`, `successfulPaymentsCurrentMonth`, and `Tracked signed-in app/device records`. `Active Premium users now` remains an entitlement/access-state metric based on currently active Premium access and does not change the paid-entitlement start semantics for users who purchase during an active Trial. Separate `Successful payments total` and `Successful payments current month` metrics count successful payment records from the internal provider-agnostic payments table, using completed Premium `transaction.completed` records with completed timestamps rather than active entitlements, subscription snapshots, raw Paddle webhook payloads, or distinct-user inference. The current-month boundary is UTC calendar month start inclusive to next UTC month start exclusive. These aggregate payment metrics do not expose emails, user IDs, Paddle customer IDs, transaction IDs, raw provider payloads, signatures, or other personal data in the Admin UI. Production/live Paddle readiness remains deferred, and broad public production readiness is not claimed.

## Website CMS draft storage current state (2026-06-27)

Website CMS can now store admin-only draft bodies for initialized sections. The Admin Website tab can select a section, load detail, and save draft body, internal notes, effective date, review status, and a required change reason. Saves are guarded against obvious secret-like/private/provider identifiers.

Saving a draft still does not publish content, update public website rendering, or modify static `site/public` rendering. There is still no published Website CMS content, no Website CMS public rendering, no public unauthenticated Website CMS route, and no live Paddle enablement. Draft copy is not final legal advice or legal approval.
