...
Current runtime-status result on the deployed backend is `effectiveSource=StaticJson`; learner runtime remains static JSON by default. Runtime validation currently reports `validationSuccess=false` because it expected 2 tutor behavior profiles but found 3. This is the next content/runtime validation issue to resolve before enabling CMS published-snapshot runtime for learners, not a production outage because CMS runtime is not enabled by default. The `tools/smoke_cms_runtime_status.ps1` smoke script default is the server-only backend `https://api.languagevoicetutor.com`; localhost must be passed explicitly only for approved local developer runs.
