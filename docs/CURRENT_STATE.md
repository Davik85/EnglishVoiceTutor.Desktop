...
This is repository state, not deployment evidence. The authenticated Google Play verification service now passes only verified trusted metadata into the atomic ownership/Subscription/entitlement persistence boundary. When enabled, a verified purchase can create or select its exact `google_play` Subscription and persist its linked Premium entitlement; test purchases remain unsupported, no acknowledgement occurs, and no Payment or BillingEvent projection is created. Google Play remains disabled by default, the implementation is not deployed, and migration `20260727045935_AddGooglePlayPurchaseClaims` remains unapplied in production. Repository-only account-status multi-provider selection is complete: active Premium and effective expiry are entitlement-driven, and provider metadata follows the selected linked entitlement or a stable safe fallback for legacy unscoped `provider_event` grants. A newer or shorter Google Play Subscription cannot hide, shorten, or replace valid Paddle Premium. Paddle checkout, webhook processing, Premium activation/cancellation/refund behavior, and Desktop Premium-expiry display remain the working website/Desktop path; Google Play must join, not replace, that shared entitlement source of truth. Production remains `0.1.35-backend.133`; the current backend release is `0.1.35-backend.133`, and commits after `.133` do not change that release claim. Credentials, package/product configuration, migration application, backend release, sandbox testing, Mobile connection, acknowledgement, replacement-token handling, RTDN, and deployment remain pending.
