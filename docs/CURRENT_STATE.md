...
Website CMS can now manage admin-only draft workflow for initialized sections. The Admin Website tab supports metadata overview, missing-section initialization, section detail, draft-body save, stored-draft validation, admin-only safe-text preview, and internal review-status changes with required `ChangeReason`. Save and validation use `WebsiteCmsContentGuard`; validation reads the stored draft and does not write to the database. Preview is admin-only and does not publish or update public rendering. Review-status changes are internal only and do not update `PublishedBody` or `PublishedAtUtc`.
...
There is still no Website CMS publish workflow, no Website CMS public rendering, no public unauthenticated Website CMS route, and no live Paddle enablement. Public rendering remains static `site/public`. Draft copy is not final legal advice or legal approval; final legal/seller/support/pricing copy still requires owner/legal approval.
