...
Final state: this was a successful rehearsal, not a permanent fallback removal. The setting was restored to `true`, `languagevoicetutor-backend.service` was restarted, and final validation passed with `ExpectedFallbackEnabled true`. Final status showed `fallbackEnabled=True`, `defaultFallbackEnabled=True`, `configValuePresent=True`, `persistentRoleAuthorizationEnabled=True`, and `actorMappingFound=True`. Production fallback is currently enabled explicitly through `backend.env`. Production Admin RBAC is stronger than before because two persistent `super_admin` accounts are verified and persistent-role authorization was proven to work with fallback disabled.

Public release is still not complete. The remaining Production Admin RBAC owner decision is whether to keep BootstrapAdmin fallback enabled as a documented temporary exception or schedule a separate owner-approved permanent fallback disable window. Non-owner role validation and critical-change approval remain future work.
...
Fallback to packaged static JSON remains available for rollback/safety, but fallback should not be active during normal runtime status. Normal status should show `effectiveSource=CmsPublishedSnapshot`, `validationSuccess=Yes`, `fallbackUsed=No`, no errors, and no warnings. Production Admin RBAC cutover rehearsal passed on 2026-06-22, but fallback is currently enabled explicitly through backend.env; the owner must still decide whether to keep that as a temporary exception or schedule a permanent fallback-disable window. Critical-change approval remains future work.
