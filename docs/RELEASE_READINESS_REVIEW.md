...
- Backend `0.1.35-backend.80` is deployed at `/opt/languagevoicetutor/backend/releases/0.1.35-backend.80` and production `/health` plus `/api/health/database` return `200 OK`.
...
- Verify backend symlink still resolves to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.80` and `/health` plus `/api/health/database` are green.
