...
- `GooglePlayPurchaseVerificationService` is connected to `IGooglePlayVerifiedPurchasePersistenceService`. The atomic orchestration establishes token-fingerprint ownership, selects or creates the exact `google_play` Subscription, and persists its linked entitlement state in one serializable transaction. This disabled infrastructure is deployed through production backend `.139`; migrations `20260727045935_AddGooglePlayPurchaseClaims`, `20260802154345_AddGooglePlayRtdnPersistenceFoundation`, and `20260803052655_AddGooglePlayPendingRefundReviewFoundation` are applied. Runtime purchase processing remains dormant because Google Play is disabled; no Google Play claim, RTDN event, reconciliation item, pending-refund review, or entitlement has been created in production. Pending-refund messages deliberately return HTTP 503 until separately approved enablement preparation and controlled validation occur. Test purchases remain unsupported. Repository-only account-status multi-provider selection is complete: valid active Premium entitlement coverage determines access and expiry; provider metadata comes only from the selected linked Subscription, or from a stable safe legacy fallback for an unscoped `provider_event` entitlement. A newer or shorter Google Play Subscription cannot hide or replace valid Paddle Premium. Paddle production behavior remains unchanged.
