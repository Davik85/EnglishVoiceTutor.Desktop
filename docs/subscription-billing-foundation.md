...
## Local Google Play RTDN reconciliation foundation

Protected Google Play purchase-token persistence, authenticated RTDN receipt, reconciliation, and linked-purchase-token replacement handling are implemented locally. Immediate and bounded deferred replacements are supported; unsupported multi-item/add-on shapes fail closed. Old linked claims remain ownership records while their protected secrets are superseded. The worker is disabled by default and reuses the authenticated purchase-processing flow; it does not establish a new entitlement policy. Pending-refund review notifications intentionally return 503 until protected pending-refund token handling and ReviewRefund work are separately approved. The RTDN persistence migration exists locally but is not applied to production, which remains backend `0.1.35-backend.138`; no Google Cloud, Play Console, migration application, deployment, or production enablement has occurred. Protected pending-refund processing remains a separate local prerequisite before production RTDN enablement.
