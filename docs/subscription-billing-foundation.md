...
- Exactly-once recovery stores the one command before provider mutation. Unknown outcomes and ETag conflicts re-read provider state: exact target expiry converges to provider-applied handling, exact baseline plus the unchanged stored command ETag permits only a bounded retry of that same command, and any other conclusive identity, ETag, or expiry divergence becomes ambiguous-terminal with no further mutation. After either a successful defer response or an unknown outcome later proven at the exact target, a temporarily unavailable or not-yet-usable GET remains retryable with bounded GET-only reconciliation; post-mutation convergence does not reapply initial deferral-eligibility fields that were required only before the command. The defer response remains supporting evidence only: the existing exact Google persistence path receives the new expiry only after a fresh authoritative GET confirms the same purchase and exact target. If that confirming snapshot is already future-expiry `CANCELED`, persistence retains the cancellation lifecycle and access through the target instead of restoring `ACTIVE`. No local baseline-plus-duration entitlement is synthesized.
- Controlled Google Play Billing, RTDN, and reconciliation are enabled for the approved Internal-testing license-test context; pending-refund review remains disabled. The trial-deferral migration and backend `.142` deployment are complete. Backend Data Protection and Android Publisher authentication are subsequently provisioned; the controlled purchase, provider mutation, accelerated renewals, and final expiry are verified. Public rollout and real-money production purchase remain pending.
