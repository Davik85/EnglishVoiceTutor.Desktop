...
Protected Google Play purchase-token persistence, authenticated RTDN receipt, reconciliation, linked-purchase-token replacement handling, the isolated pending-refund review pipeline, the trial-deferral foundation, and account-wide purchase-gating backend support are deployed in backend `0.1.35-backend.142`. Migrations `20260802154345_AddGooglePlayRtdnPersistenceFoundation`, `20260803052655_AddGooglePlayPendingRefundReviewFoundation`, and `20260827105749_AddGooglePlayTrialDeferralFoundation` are applied. The pending-refund module protects only token/order payloads, retains terminal protected material only for bounded cleanup, supports only explicit `NEUTRAL`, and does not fabricate usage evidence or directly alter Premium. Android Publisher production authentication is provisioned through a service-account credential outside versioned release directories; OAuth and read-only Android Publisher provider access are verified. Google Play Billing, RTDN, reconciliation, and pending-refund review remain disabled in production. Google Cloud Pub/Sub / RTDN production configuration, Play Console base-plan activation, Mobile store release, purchases, and Google provider mutations have not occurred; Paddle behavior is unchanged except for deployed defensive backend ownership/isolation semantics.
...
- Production Google Play Billing, RTDN, reconciliation, and pending-refund processing remain disabled. The trial-deferral migration and backend `.142` deployment are complete. Backend Data Protection and Android Publisher authentication are subsequently provisioned; OAuth and read-only Android Publisher provider access are verified, but Pub/Sub setup in Google Cloud and RTDN configuration, Play activation, purchases, provider mutations, and runtime enablement have not occurred.
