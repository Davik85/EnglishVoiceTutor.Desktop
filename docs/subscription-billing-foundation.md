...
Production backend `0.1.35-backend.146` is deployed and healthy, but validation did **not** close the initial Premium deferral blocker. A fresh explicitly allowed license-test purchase was verified and acknowledged, and its single defer mutation returned the exact immutable target. The immediate post-defer path nevertheless marked the plan `ambiguous_terminal` with `trial_deferral_provider_state_diverged` and no authoritative provider expiry; a normal subscriptions-v2 verification only seconds later persisted the exact target. Related RTDN rows became `permanent_failure` / `provider_rejected` although the user had not canceled. This independently reproduced the same post-defer failure class previously seen with a real-money purchase.

The evidence proves exact-target acceptance followed by later authoritative convergence, but it does not prove that expiry was the only field differing in the first post-defer snapshot. The source correction therefore keeps only same-purchase, valid-lifecycle non-convergence retryable, preserves conclusive identity contradictions as terminal, performs GET-only bounded retries after exact-target defer-response evidence, and completes only from a fresh authoritative exact-target snapshot. Public rollout remains blocked until that correction is deployed and controlled production validation passes. No `.147` deployment, row repair, migration, configuration change, Play Console change, or new provider mutation is claimed.
...
- Exactly-once recovery stores the one command before provider mutation. Unknown outcomes and ETag conflicts re-read provider state: exact target expiry converges to provider-applied handling, while exact baseline plus the unchanged stored command ETag remains the only path that permits a bounded retry of that same command. After a successful defer response proves the exact immutable target, a same-purchase, valid-lifecycle baseline or intermediate expiry remains bounded and GET-only retryable; conclusive start/product/linked-purchase identity contradictions remain ambiguous-terminal with no further mutation. Temporarily unavailable or structurally unusable GETs also remain bounded and GET-only. Safe assessment logs expose classification only: `target_converged`, `baseline_not_yet_converged`, `intermediate_expiry_not_yet_converged`, `temporarily_unusable`, or `identity_contradiction`. The defer response remains supporting evidence only: the existing exact Google persistence path receives the new expiry only after a fresh authoritative GET confirms the same purchase and exact target. If that confirming snapshot is already future-expiry `CANCELED`, persistence retains the cancellation lifecycle and access through the target instead of restoring `ACTIVE`. No local baseline-plus-duration entitlement is synthesized, and non-convergence fails closed at the existing retry limit.
- Controlled Google Play Billing, RTDN, and reconciliation are enabled for the approved Internal-testing license-test context; pending-refund review remains disabled. The trial-deferral migration and backend `.142` deployment are complete, and production has since advanced to `.146`. Backend Data Protection and Android Publisher authentication are provisioned; controlled purchase, provider mutation, accelerated renewals, final expiry, and a real-money first purchase are verified. Initial-deferral post-mutation convergence remains a public-rollout blocker until the corrected source behavior is deployed and controlled validation passes.
