...
- Production Google Play Billing, RTDN, reconciliation, and pending-refund processing remain disabled. The trial-deferral migration and backend `.142` deployment are complete. Backend Data Protection was subsequently provisioned and enabled, but no Google credentials, Pub/Sub/Google Cloud setup, Play activation, provider call, or runtime enablement has occurred.
