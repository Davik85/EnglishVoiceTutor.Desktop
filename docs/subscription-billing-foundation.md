...
Recovery re-runs reconciliation with the current fallback user-resolution logic: metadata internal user id first, then `PaymentEntity.ProviderPaymentId` / Paddle transaction id, then `SubscriptionEntity.ProviderSubscriptionId` / Paddle subscription id, then active provider-event Premium entitlement evidence linked through a known provider subscription where supported by the entitlement activation path. Full refunds and chargebacks revoke active provider-event Premium; partial refunds are conservative and do not automatically revoke Premium. The operation is idempotent: if Premium is already revoked or no active provider-event Premium entitlement remains, the result is `AlreadyRevoked`; unknown event ids return `NotFound`; unsafe mapping or reconciliation failures return a blocked result; and non-`adjustment.created`/`adjustment.updated` events are refused.

Production note: backend `.97` received duplicate replay of Paddle `adjustment.updated` event `evt_01kwhgmvh1v9k8ve70gvnfeskm`, but duplicate ingestion was idempotent and did not reprocess the already-normalized/skipped event. Premium revocation remains not production-verified until this recovery/reprocess path is deployed and run. No more live payment/refund testing should be performed; Manual Premium Revoke remains the operational stopgap if immediate protection is needed. Broad public paid launch remains pending. Customer portal remains pending unless separately verified. Direct installer code signing remains pending.
