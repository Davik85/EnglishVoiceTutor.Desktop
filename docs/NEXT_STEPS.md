...
The paragraphs below preserve the `.142` rollout baseline as historical context. The current source of truth is that controlled Internal-testing purchase, renewal reconciliation, and final expiry have completed; Google Play Billing, RTDN, and reconciliation are enabled only for that approved context, while pending-refund review remains disabled and public rollout remains pending.
...
The controlled Google Play purchase-verification, RTDN/reconciliation/linked-purchase/pending-refund, account-wide Premium, and purchase-gating foundations are deployed in production `.142`; `.141` is retained as rollback. Controlled purchase, renewal reconciliation, and final expiry are verified, but they are not broad public rollout readiness. Persistent Backend Data Protection setup and Android Publisher authentication remain complete as previously recorded.

The next work is: (A) synchronize Privacy, Terms, seller/company, refund, cancellation, subscription, and applicable AI/speech/transcription public wording; (B) re-review Google Play Data Safety against the current app and public policies; (C) before public rollout, review test-only controls/allowlist, whether `TestPurchasesEnabled` must be disabled, pending-refund-review policy, monitoring, and rollback readiness; (D) prepare a new Mobile Internal-testing release with a new versionCode so signed-out password recovery is distributed; (E) separately build/sign/verify and manually upload that AAB; (F) run a Play-installed Login/registration/Forgot-password/reset/sign-in/Premium/basic-billing/lesson-voice smoke; and (G) make any production rollout decision separately. Do not change those runtime settings in documentation work.
