...
## 2026-09-04 post-release monitoring

Android v8 is publicly available in Google Play Production after owner approval, review submission, and confirmed public installation. Backend `0.1.35-backend.155` is current with `.154` rollback; the service is active and public backend/database health checks returned HTTP 200. `.155` was deployed from source commit `3821c02f284c291d47815157b97f906f4b4956d9` with no EF migration. Enabled Google Play Billing/RTDN/reconciliation continuity, the protected RTDN endpoint, the independent lightweight homepage (`index.html` plus `assets/homepage/`), legacy `mobile.html` redirect, retained `styles.css`, and public legal/support URLs remain established production state.
...
The paragraphs below preserve the `.139`/`.142` rollout baseline as historical context. The current source of truth is production backend `0.1.35-backend.155` with `.154` rollback and Mobile `0.1.0+8` / versionCode 8 publicly available in Google Play Production. Google Play Billing, RTDN, and reconciliation remain enabled; pending-refund review remains separately scoped.
...
The controlled Google Play purchase-verification, RTDN/reconciliation/linked-purchase/pending-refund, account-wide Premium, and purchase-gating foundations were deployed in historical `.142`; `.141` was its rollback reference. Controlled purchase, renewal reconciliation, and final expiry are verified, but they are not broad public rollout readiness. Persistent Backend Data Protection setup and Android Publisher authentication remain complete as previously recorded.

The active sequence is: (1) documentation synchronization — complete; (2) public legal/privacy/support synchronization — complete; (3) Google Play Data Safety re-audit/submission — complete, with Google review still pending; (4) Restore Credentials implementation and cross-device E2E — complete; (5) the 2026-09-01 purchase-gate/first-purchase checks and `.148` initial-deferral provider-precision validation — complete; (6) resolve any remaining concrete lifecycle, pending-refund, or Google-review issues; (7) select the Production candidate (versionCode 8 remains viable absent a Mobile change); (8) run only the targeted Play-installed smoke justified by changes since v8; (9) present the final Production release summary; (10) obtain explicit owner approval before any Start rollout to production action; and (11) after publication, perform targeted production monitoring.

Any future read-only configuration gate must inspect the live `TestPurchasesEnabled` setting, exact test-account/allowlist/test-only gates, enabled states and relevant live parameters for Google Play Billing/RTDN/reconciliation, remaining development or debug purchase controls, pending-payment, refund/voided-purchase/revoke, cancellation/expiry, and pending-refund-review assumptions, backend and Google Play monitoring/log visibility, health checks, and the current backend rollback target and operational rollback path. It must not flip settings blindly: any production configuration change requires separately reviewed action and explicit owner approval.

Do not assume that another AAB is required merely because v8 is in Internal testing. If this gate and the pending Data Safety review require no Mobile artifact change, versionCode 8 may be selected from the Play library as the first Production candidate. If a Mobile code or bundle change is actually required, the next uploaded build must use versionCode 9 or later and follow the normal build/sign/hash/Internal-testing/targeted-smoke path before a Production decision. The first Production publication has no normal staged-rollout percentage control; before the separate, irreversible public Start rollout to production action, the decision must explicitly cover the candidate, countries/regions, Data Safety/review state, open risks, backend and Google Play billing/RTDN/reconciliation monitoring, and rollback/recovery.

Production `.142` includes the applied additive migration `20260827105749_AddGooglePlayTrialDeferralFoundation` and source support for the exactly-once initial account-wide Premium deferral workflow for Product ID `premium`. Applying the migration, controlled runtime enablement, and validation are no longer pending work. Exact Base Plan ID `monthly` is active, with no Google Play free-trial or introductory offer: the seven-day trial remains backend-owned, contiguous already-owned manual/provider Premium is preserved, and positive sub-day required defer uses the documented 24-hour minimum for `purchases.subscriptionsv2.defer`. Provider expiry remains authoritative after a fresh GET; no local target expiry may be synthesized from the stored duration.

That later work must preserve valid trial, `manual_admin`, and Paddle entitlement plus the final Desktop/Mobile Premium expiry; a Google Play failure must never revoke, shorten, hide, replace, or relabel that Premium. Provider-specific metadata selection must remain separate from provider-neutral Premium coverage calculation. The [mandatory billing-adapter regression gate](subscription-billing-foundation.md#mandatory-billing-adapter-regression-gate) applies before future Google Play changes are accepted. For the approved controlled context, `GooglePlayBilling.Enabled=true`, `GooglePlayRtdn.Enabled=true`, and `GooglePlayReconciliation.Enabled=true`; `GooglePlayPendingRefundReview` remains disabled. The controlled Internal-testing license-test purchase, accelerated renewals, and final expiry are verified.

The 2026-09-01 real-money first purchase and existing backend-owned initial Premium/trial deferral are also verified: a non-license-test account completed a normal paid Google Play purchase, backend-confirmed Premium remained active, and fresh provider-management state showed the deferred next payment on 2026-10-08. This is not a Google Play trial or introductory offer; it preserves the backend registration trial and continuous Premium tail through the existing initial deferral mechanism. The actual 2026-10-08 renewal charge, pending payment, explicit pre-expiry cancellation, fresh-install billing restore, refund/voided-purchase, chargeback, and public rollout remain pending. Paddle production behavior remains unchanged.

Backend `.146` production validation did not close the initial-deferral blocker. For a fresh explicitly allowed license-test purchase, `DeferAsync` returned the intended target at provider precision, but the immediate post-defer convergence path still marked the plan `ambiguous_terminal`; a normal subscriptions-v2 verification seconds later persisted that same provider expiry. This independently reproduced the post-defer failure class seen with the real-money purchase. The first post-defer snapshot was not classified with enough safe detail to prove expiry was its only difference. Do not repair the existing evidence rows as part of the source correction.

Backend `.147` correctly retained the first genuine intermediate post-defer snapshot as bounded GET-only pending, but strict timestamp equality rejected Google’s provider-normalized target. The `.148` checkpoint above records the fixed narrow equivalence behavior and its successful controlled validation. Do not repair the existing `.146` or `.147` evidence rows; public Production remains a separately approved decision.

Backend `.142` scopes Paddle activation, lifecycle expiry, full-refund, and chargeback mutation to the exact Paddle-owned `Subscription`/linked entitlement. Ambiguous legacy `provider_event` rows with `SubscriptionId == null` are never retroactively claimed by Paddle and fail closed for manual review; Google, `manual_admin`, trial, and other Paddle subscription coverage are not Paddle mutation targets. This deployed defensive correction is not a provider-runtime enablement claim.

Historical backend `.142` includes bounded provider-renewal purchase gating. The additive authenticated status fields are calculated from all relevant Premium provider subscriptions rather than the single provider snapshot selected for display. Active/trialing Paddle is confirmed non-renewing only when the subscription's latest provider event matches a processed Paddle `BillingEvent` whose safe metadata proves the same user/subscription/event identity, a complete scheduled-change snapshot, `cancel`, and a strictly future effective time. Legacy or otherwise unproven sticky cancellation fields fail closed, as does a past effective time while local state remains active/trialing. Paddle lifecycle persistence treats a complete `subscription.updated` snapshot with explicit `scheduled_change: null` as authoritative removal of an earlier cancel/pause/resume; missing or invalid scheduled-change evidence never implies removal. Active renewal after removal and every recoverable payment-retry/past-due state remain blocking even when cancellation metadata is also present; paused/resumable state blocks, and multiple, incomplete, or conflicting ownership fails closed. The current Play-distributed versionCode 8 includes signed-out password recovery. No provider is automatically canceled, converted, or switched.
...
1. **Notifications:** device-local Practice reminders are implemented; they do not imply remote push, Firebase, a backend endpoint, push-token registration, remote-provider state, or background microphone behavior.
2. **Premium purchase entry points and purchase flow:** the controlled Google Play flow is implemented. Premium remains backend-owned and backend-verified; a local button press or unverified store result never unlocks it. No Paddle change is part of Mobile Google Play work.
3. **Interface localization:** fourteen selectable interface locales are implemented, distinct from the six study languages. Localization does not translate backend-generated tutor replies, CMS canonical IDs, lesson runtime metadata, or user content.

Mobile remains the same backend account/product, with backend-owned Premium, no client-side OpenAI, no Mobile secrets or database, no unapproved endpoint, and no provider or production action implied by planning.
...
The Website CMS now includes a visible **Marketing / SEO** section with consent-banner, analytics, ads, a generic Search Console token field, and `llms.txt` controls. These values are stored through the existing JSON/file-based Website CMS model; no database table, schema change, migration, backend secret, env value, or committed example JSON value is required for Google setup. Search Console ownership for `languagevoicetutor.com` is already verified through DNS/domain-provider verification, so the current production site does not use an HTML token or homepage `google-site-verification` meta tag.

Current production CMS marketing state: consent banner and analytics are ON with a valid GA4 Measurement ID configured only in Website CMS; Google Ads tracking is OFF with an empty Ads ID and conversion label. Search Console is closed DNS-verified work; its generic CMS HTML-token field is not used.

Operator field guide: GA4 Measurement ID comes from Google Analytics → Admin → Data streams → Web stream for `languagevoicetutor.com` and has format `G-XXXXXXXXXX`; Google Ads ID comes from Google Ads conversion tag setup and has format `AW-123456789`; the download conversion label comes from the same conversion action setup. Do not obtain or paste a Search Console HTML verification token for the current DNS-verified property, and do not add a homepage verification meta tag. Do not paste placeholders, whole script snippets, or GTM container IDs into these fields unless GTM support is explicitly added later.

Website Publish now emits or maintains public HTML pages, `robots.txt`, `sitemap.xml`, `llms.txt` when enabled, and `marketing-consent.js`. The generated runtime provides fallback CMS analytics configuration to independent `index.html` only when that page has no inline configuration; the homepage never maintains a GA ID. Generated pages include canonical URLs, meta descriptions, Open Graph/Twitter tags, JSON-LD where appropriate, and SoftwareApplication JSON-LD for Windows desktop only.
...
Current known-good model configuration: lesson tutor chat `gpt-5.6-luna`; feedback/correction `gpt-5.6-luna`; lesson hint `gpt-5.6-luna`; translation `gpt-5.6-luna`; all four text-role omit-temperature flags enabled; speech-to-text `gpt-4o-mini-transcribe`; lesson chat TTS `tts-1`; Conversation Mode TTS `gpt-4o-mini-tts`; Realtime voice `gpt-realtime`.
...
The earlier `gpt-5.5` lesson tutor chat root cause was unsupported `temperature`, not provider unavailability. Minimal Responses API text passed, minimal structured output passed, and lesson runtime shape without user content passed after `temperature` was omitted. Compatibility coverage preserves the established `gpt-5.2` numeric-temperature behavior where applicable and the existing `gpt-5.5` omission behavior. The current `gpt-5.6-luna` Active configuration uses the explicit omit flags; do not assume every newer model accepts parameters accepted by older models.

Completed deployment state: `.152` added four independent Draft/Active omit-temperature flags for lesson tutor chat, feedback/correction, lesson hint, and translation; `.153` applied the Lesson Tutor Chat policy to Lesson Summary; and `.154` completed independent runtime routing for all four text roles. Missing or `false` retains the role's legacy request behavior; `true` removes the property. Draft Save, Validate, Test provider access, Reset, and Publish retain the flags, and provider/runtime temperature policy is evaluated against the exact selected role model. Lesson Summary intentionally remains on the Lesson Tutor Chat model and flag. Audio, TTS, and Realtime roles remain unchanged, and no flag is enabled automatically from a model name. This work is complete and is no longer a future routing limitation.

Compatibility diagnostics should be read as a matrix: `minimal_responses_text` checks model availability / Responses API access without `temperature`; `current_provider_test_shape` checks the lesson tutor role's effective minimal provider-test shape and includes `temperature` only when its legacy policy and Draft override resolve to a number; `minimal_structured_output` checks strict structured output without `temperature`; and `lesson_chat_runtime_shape_without_user_content` checks lesson runtime request options/schema with safe dummy input and the same effective-temperature policy. If a new model breaks lessons, inspect safe backend logs for operation, model role, configured model ID, provider status/category, and safe provider error type/code/param/message, then restore a previous known-good model if needed. Logs and Admin UI must not expose secrets, raw provider bodies, raw request bodies, full prompts, private user lesson text, env values, or connection strings.
...
10. Keep backend current-state docs aligned with current production: production is `0.1.35-backend.145`; verify the live rollback target, `/health`, and `/api/health/database` before any separate backend operation. The 2026-07-11 `.112`/`.111` verification remains historical only.
