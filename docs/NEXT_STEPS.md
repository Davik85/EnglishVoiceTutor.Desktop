...
The paragraphs below preserve the `.139`/`.142` rollout baseline as historical context. The current source of truth is production backend `0.1.35-backend.146`, Mobile `0.1.0+8` / versionCode 8 in Google Play Internal testing, and the completed Restore Credentials cross-device E2E for the tested Android/Google Play account-session path. Google Play Billing, RTDN, and reconciliation are enabled only for the approved controlled context, while pending-refund review remains disabled and public rollout remains blocked by initial-deferral convergence validation.
...
Backend `.146` production validation did not close the initial-deferral blocker. For a fresh explicitly allowed license-test purchase, `DeferAsync` returned the exact immutable target, but the immediate post-defer convergence path still marked the plan `ambiguous_terminal`; a normal subscriptions-v2 verification seconds later persisted that exact target. This independently reproduced the post-defer failure class seen with the real-money purchase. The first post-defer snapshot was not classified with enough safe detail to prove expiry was its only difference. Do not select or start the public Production rollout until corrected convergence code is reviewed and deployed and a controlled validation passes. Do not repair the existing evidence rows as part of the source correction.
...
- Backend: production healthy at `https://api.languagevoicetutor.com` on `0.1.35-backend.146`. Historical `.142` applied the additive Google Play trial-deferral foundation and deployed the account-wide Premium/purchase-gating foundation; `.144` added safe Restore Credentials diagnostics, `.145` corrected Restore Credentials registration user verification, and `.146` did not close the remaining post-defer convergence blocker. Restore Credentials is enabled with its applied foundation migration and live assetlinks association. One free lesson per day, usage telemetry, and separate technical rate limiting remain intact. CMS published version `51` remains the valid `CmsPublishedSnapshot` runtime source with `fallbackUsed=false`, 26 scenarios, 130 localized setup-message templates, and 625 localized context titles. Controlled Google Play Internal-testing runtime is enabled, but public Google Play rollout remains blocked pending corrected convergence deployment and controlled validation; the complete account-deletion workflow, CMS/Admin login security, and persistent role management remain production-verified.
