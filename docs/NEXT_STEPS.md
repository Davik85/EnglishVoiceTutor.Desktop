...
The paragraphs below preserve the `.139`/`.142` rollout baseline as historical context. The current source of truth is production backend `0.1.35-backend.145`, Mobile `0.1.0+8` / versionCode 8 in Google Play Internal testing, and the completed Restore Credentials cross-device E2E for the tested Android/Google Play account-session path. Google Play Billing, RTDN, and reconciliation are enabled only for the approved controlled context, while pending-refund review remains disabled and public rollout remains pending.
...
- Backend: production healthy at `https://api.languagevoicetutor.com` on `0.1.35-backend.145`. Historical `.142` applied the additive Google Play trial-deferral foundation and deployed the account-wide Premium/purchase-gating foundation; `.144` added safe Restore Credentials diagnostics and `.145` corrected Restore Credentials registration user verification without a further EF migration. Restore Credentials is enabled with its applied foundation migration and live assetlinks association. One free lesson per day, usage telemetry, and separate technical rate limiting remain intact. CMS published version `51` remains the valid `CmsPublishedSnapshot` runtime source with `fallbackUsed=false`, 26 scenarios, 130 localized setup-message templates, and 625 localized context titles. Controlled Google Play Internal-testing runtime is enabled, and the complete account-deletion workflow, CMS/Admin login security, and persistent role management remain production-verified.
