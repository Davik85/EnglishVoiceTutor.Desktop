...
The paragraphs below preserve the `.139`/`.142` rollout baseline as historical context. The current source of truth is production backend `0.1.35-backend.147`, Mobile `0.1.0+8` / versionCode 8 in Google Play Internal testing, and the completed Restore Credentials cross-device E2E for the tested Android/Google Play account-session path. Google Play Billing, RTDN, and reconciliation are enabled only for the approved controlled context, while pending-refund review remains disabled and public rollout remains blocked by initial-deferral provider-precision convergence validation.
...
The active sequence is: (1) documentation synchronization — complete; (2) public legal/privacy/support synchronization — complete; (3) Google Play Data Safety re-audit/submission — complete, with Google review still pending; (4) Restore Credentials implementation and cross-device E2E — complete; (5) the 2026-09-01 purchase-gate and first-purchase checks — complete, but initial-deferral convergence remains blocked after `.147`; (6) review, deploy, and controlled-production-validate the fixed, strictly-less-than-one-millisecond Google target-equivalence rule; (7) resolve any remaining concrete lifecycle or Google-review issues; (8) select the Production candidate; (9) run only the targeted Play-installed smoke justified by changes since v8; (10) present the final Production release summary; (11) obtain explicit owner approval before any Start rollout to production action; and (12) after publication, perform targeted production monitoring.
...
Backend `.147` then correctly retained the first genuine intermediate post-defer snapshot as bounded GET-only pending, but still failed validation because strict timestamp equality rejected Google’s provider-normalized target. Production evidence showed the `DeferAsync` response and later authoritative GET agreed exactly with each other and were both 460 microseconds earlier than the internally consistent local target. The next correction must remain fixed and narrow: UTC difference strictly below one millisecond is target-equivalent only for Google trial-deferral convergence, while one millisecond or more remains non-equivalent; persistence must use the actual authoritative Google expiry. Do not select or start the public Production rollout until that correction is reviewed, deployed, and controlled validation passes. Do not repair the existing `.146` or `.147` evidence rows in this source task, and do not claim `.148` is deployed.
...
- Backend: production healthy at `https://api.languagevoicetutor.com` on `0.1.35-backend.147`. Historical `.142` applied the additive Google Play trial-deferral foundation and deployed the account-wide Premium/purchase-gating foundation; `.144` added safe Restore Credentials diagnostics, `.145` corrected Restore Credentials registration user verification, `.146` exposed the post-defer convergence blocker, and `.147` handled intermediate snapshots but failed on Google’s 460-microsecond timestamp normalization. Restore Credentials is enabled with its applied foundation migration and live assetlinks association. One free lesson per day, usage telemetry, and separate technical rate limiting remain intact. CMS published version `51` remains the valid `CmsPublishedSnapshot` runtime source with `fallbackUsed=false`, 26 scenarios, 130 localized setup-message templates, and 625 localized context titles. Controlled Google Play Internal-testing runtime is enabled, but public Google Play rollout remains blocked pending provider-precision correction deployment and controlled validation; the complete account-deletion workflow, CMS/Admin login security, and persistent role management remain production-verified.
