...
Production tester builds must use `https://api.languagevoicetutor.com`; Release builds must not silently use localhost, 127.0.0.1, empty URLs, or stale local development overrides. Clean-machine smoke must verify health, registration, login, settings sync, lesson start, history, progress, password reset, and update check from a real installed build. Tester handoff is blocked until the installed-build backend connectivity issue is verified fixed on a second Windows device.
...
- No automatic update-check or auto-updater exists. The simple manual update check reads the direct-download `latest.json` manifest only after user action, requires manual confirmation, verifies SHA-256 before starting the installer, and must not interrupt active lessons. Clean-machine manual update check verification remains required before external tester handoff.
...
- Continue next with password recovery/reset, signed-in password change, server-connected CMS verification, a public download page, manual update check verification, clean-machine tester smoke, and only then controlled tester handoff; keep production billing deferred and public release not ready.
...
- External tester handoff remains blocked until CMS/Admin server verification, manual update check verification and clean-machine smoke, and checklist completion are finished.
...
- External tester handoff remains blocked until CMS/Admin server verification, manual update check verification and clean-machine smoke, and checklist completion.

## Static tester download page follow-up

A basic public download page foundation is now prepared under `site/public/`. The page is static and reads `latest.json` from the existing Windows direct release folder at `/releases/windows/direct/latest.json`; it derives the **Download for Windows** link only from manifest `installerRelativeUrl`, displays manifest `version`, and shows manifest `installerFileName` in release details. It does not implement auto-update and complements the in-app manual update UI. It is only a tester download page for invited testers.

Before external tester handoff, deploy the static page only after reviewing `scripts/upload-static-site.ps1` with `-DryRun`, verifying the public HTTPS page loads the manifest, and confirming there is no hardcoded old installer fallback if the manifest request fails. After every Windows release upload, verify both the displayed current version and the actual downloaded installer filename. External tester handoff is still blocked until the manual update check verification and the clean-machine smoke checklist pass.
...
## Update UX and history preservation follow-up

The desktop now has a simple manual update check from normal Settings, not just Diagnostics. It validates `latest.json`, compares versions, asks before downloading/installing, verifies SHA-256 before starting the installer, and never performs a silent auto-update. Keep future update work within this manual-confirmation safety model unless a separate signed updater design is approved.
