...
The following sequence records the then-planned release steps: (1) documentation synchronization — complete; (2) public legal/privacy/support synchronization — complete; (3) Google Play Data Safety re-audit/submission — complete, with Google review then pending; (4) Restore Credentials implementation and cross-device E2E — complete; (5) the 2026-09-01 purchase-gate/first-purchase checks and `.148` initial-deferral provider-precision validation — complete; (6) resolve any remaining concrete lifecycle, pending-refund, or Google-review issues; (7) select the Production candidate; (8) run only the targeted Play-installed smoke justified by changes since v8; (9) present the final Production release summary; (10) obtain explicit owner approval before any Start rollout to production action; and (11) after publication, perform targeted production monitoring. It is historical context, not a current v8 or pending SEO release plan.
...
At that historical point, another AAB was not assumed merely because v8 was in Internal testing. If the then-current gate and pending Data Safety review required no Mobile artifact change, versionCode 8 could be selected from the Play library as the first Production candidate. If a Mobile code or bundle change was required, the next uploaded build had to use versionCode 9 or later and follow the normal build/sign/hash/Internal-testing/targeted-smoke path before a Production decision. This is retained historical release-process context; Android `0.1.0+9` / versionCode 9 is now publicly available.
...
Normal flow: load draft/active → Save draft → Preview selected page without publishing → Publish / Make active to promote content and render CMS-managed static pages. Publish creates `download.html`, `pricing.html`, `support.html`, `terms.html`, `privacy.html`, `refunds.html`, `cancellation.html`, `seller.html`, `ai-data.html`, and `status.html`, and maintains `robots.txt`, `sitemap.xml`, optional `llms.txt`, and `marketing-consent.js`. It must preserve independent `index.html`, `mobile.html`, `styles.css`, the six language-practice pages, and `assets/homepage/**`.
