...
The disabled Google Play purchase-verification, RTDN/reconciliation/linked-purchase/pending-refund, account-wide Premium, and purchase-gating foundations are deployed in production `.142`; `.141` is retained as rollback. Migrations `20260727045935_AddGooglePlayPurchaseClaims`, `20260802154345_AddGooglePlayRtdnPersistenceFoundation`, `20260803052655_AddGooglePlayPendingRefundReviewFoundation`, and `20260827105749_AddGooglePlayTrialDeferralFoundation` are applied. The next stage is a separate reviewed production-enablement preparation phase for Backend Data Protection persistent key-ring/certificate setup, Android Publisher authentication/credentials, Google Cloud/Pub/Sub/RTDN and Play Console configuration, and controlled test validation. Source support for active-plus-previous Data Protection certificate decryption exists, but no certificate or persistent key ring is provisioned and Data Protection is not enabled. None of the enablement prerequisites are complete, and all modules remain disabled until separately approved.
