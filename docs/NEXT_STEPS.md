...
The RTDN, protected-token persistence, reconciliation, linked-purchase-token replacement, and protected pending-refund review database rollout was completed in historical backend `0.1.35-backend.139`. Migrations `20260802154345_AddGooglePlayRtdnPersistenceFoundation` and `20260803052655_AddGooglePlayPendingRefundReviewFoundation` are applied. All Google Play modules remain disabled; pending-refund review persists no raw token/order ID, supports only explicit `NEUTRAL`, submits no fabricated usage evidence, and never changes Premium directly.

The disabled Google Play purchase-verification, RTDN/reconciliation/linked-purchase/pending-refund, account-wide Premium, and purchase-gating foundations are deployed in production `.142`; `.141` is retained as rollback. Migrations `20260727045935_AddGooglePlayPurchaseClaims`, `20260802154345_AddGooglePlayRtdnPersistenceFoundation`, `20260803052655_AddGooglePlayPendingRefundReviewFoundation`, and `20260827105749_AddGooglePlayTrialDeferralFoundation` are applied. Persistent Backend Data Protection setup is complete: it is enabled with its approved persistent key ring and active certificate outside release directories, and its protected backup integrity and isolated restore drill are verified. Source support for active-plus-previous Data Protection certificate decryption remains available for a future replacement; no previous certificate is configured and no replacement is underway. Android Publisher authentication is complete: the production service-account credential is provisioned outside release directories, and OAuth plus a read-only Android Publisher request succeeded without a purchase or provider mutation. The next bounded pre-enablement phase is Google Cloud Pub/Sub/RTDN and corresponding Play Console configuration, followed by controlled test validation. All Google Play runtime modules remain disabled until separately approved.

Production `.142` includes the applied additive migration `20260827105749_AddGooglePlayTrialDeferralFoundation` and source support for the exactly-once initial account-wide Premium deferral workflow for Product ID `premium`. Applying the migration and deploying the backend are no longer pending work; runtime enablement remains separately prohibited until the remaining configuration/security/provider prerequisites and controlled validation are approved. Exact Base Plan ID `monthly` is saved in Play Console as a draft and is not activated. The launch base plan has no Google Play free-trial or introductory offer: the seven-day trial remains backend-owned, contiguous already-owned manual/provider Premium is preserved, and positive sub-day required defer uses the documented 24-hour minimum for `purchases.subscriptionsv2.defer`. Provider expiry remains authoritative after a fresh GET; no local target expiry may be synthesized from the stored duration.

That later work must preserve valid trial, `manual_admin`, and Paddle entitlement plus the final Desktop/Mobile Premium expiry; a Google Play failure must never revoke, shorten, hide, replace, or relabel that Premium. Provider-specific metadata selection must remain separate from provider-neutral Premium coverage calculation. The [mandatory billing-adapter regression gate](subscription-billing-foundation.md#mandatory-billing-adapter-regression-gate) applies before any Google Play work is accepted. The Google Play purchase-verification endpoint remains disabled; Google Play, RTDN, reconciliation, and pending-refund review remain disabled in production, and pending-refund messages return 503 until a separately approved enablement preparation and controlled validation sequence. Controlled accelerated license-test eligibility exists only behind disabled-by-default backend test controls; no test or production purchase was made here. Paddle production behavior remains unchanged.
...
- Backend: production healthy at `https://api.languagevoicetutor.com` on `0.1.35-backend.142`, with `.141` as rollback. Release `.141` removed obsolete product quotas without an EF migration or schema change; `.142` applied the additive Google Play trial-deferral foundation migration and deployed the account-wide Premium/purchase-gating backend foundation. One free lesson per day, usage telemetry, and separate technical rate limiting remain intact. CMS published version `51` remains the valid `CmsPublishedSnapshot` runtime source with `fallbackUsed=false`, 26 scenarios, 130 localized setup-message templates, and 625 localized context titles. Google Play runtime remains disabled, and the complete account-deletion workflow, CMS/Admin login security, and persistent role management remain production-verified.
...
10. Keep backend current-state docs aligned with current production: the live symlink is `0.1.35-backend.142`, rollback is `0.1.35-backend.141`; verify `/health` and `/api/health/database` before any separate backend operation. The 2026-07-11 `.112`/`.111` verification remains historical only.
