...
The unreleased source tree adds four independent Draft/Active omit-temperature flags for lesson tutor chat, feedback/correction, lesson hint, and translation. Missing or `false` retains the role's legacy request behavior; `true` removes the property. Draft Save, Validate, Test provider access, Reset, and Publish retain the flags, and provider testing uses the same effective-temperature policy as current runtime requests. Audio, TTS, and Realtime roles remain unchanged. This capability is not deployed and no flag is enabled automatically from a model name.
