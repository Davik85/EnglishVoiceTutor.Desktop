...
The paragraphs below preserve the `.139`/`.142` rollout baseline as historical context. The current source of truth is production backend `0.1.35-backend.154` with `.153` rollback and Mobile `0.1.0+8` / versionCode 8 publicly available in Google Play Production. Google Play Billing, RTDN, and reconciliation remain enabled; pending-refund review remains separately scoped.
