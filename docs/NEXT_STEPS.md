...
Authenticated RTDN receipt, protected token persistence, and a disabled-by-default local reconciliation worker are now implemented. Pending-refund review notifications remain deliberately retryable with HTTP 503 until an encrypted pending-refund workflow exists. Migration `20260802154345_AddGooglePlayRtdnPersistenceFoundation` is not applied to production. The next local billing stage is linked-purchase-token replacement lifecycle handling; production remains `0.1.35-backend.138`, and no Google Cloud or Play Console enablement has occurred. One controlled server-preparation, migration, and backend-deployment sequence follows only after the remaining local Google Play lifecycle work is accepted.

The deployed disabled Google Play purchase-verification foundation remains in production `0.1.35-backend.138` with `.137` retained as rollback, and migration `20260727045935_AddGooglePlayPurchaseClaims` remains applied. The newer local RTDN receipt, protected-token persistence, and reconciliation foundations remain disabled in production; their migration `20260802154345_AddGooglePlayRtdnPersistenceFoundation` is not applied. Linked-purchase-token replacement lifecycle handling is the next local implementation stage, before any Google Console, sandbox, server-preparation, migration, or deployment work.

That later work must preserve valid trial, `manual_admin`, and Paddle entitlement plus the final Desktop/Mobile Premium expiry; a Google Play failure must never revoke, shorten, hide, replace, or relabel that Premium. Provider-specific metadata selection must remain separate from provider-neutral Premium coverage calculation. The [mandatory billing-adapter regression gate](subscription-billing-foundation.md#mandatory-billing-adapter-regression-gate) applies before any Google Play work is accepted. Google Play, RTDN, and reconciliation remain disabled in production; pending-refund notifications deliberately return HTTP 503 until protected handling exists. Test purchases remain unsupported; Paddle production behavior remains unchanged.
