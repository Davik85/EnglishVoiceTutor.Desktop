...
Backend `.98` operator reprocess was run for existing provider event id `evt_01kwhgmvh1v9k8ve70gvnfeskm` and returned `Result=Blocked` / `BlockReason=reconciliation_blocked`; root cause was reprocess still depending on the old blocked reconciliation state. Next operator step after `.99` deployment: run the same operator-only Paddle adjustment reprocess command for `evt_01kwhgmvh1v9k8ve70gvnfeskm`; do not create a new payment, do not create a new refund, and do not perform another Paddle replay or live billing test. Broad public paid launch remains pending until the command returns `Revoked` or `AlreadyRevoked` and Desktop/Admin confirm Premium is inactive; customer portal verification and Direct installer code signing remain pending.
