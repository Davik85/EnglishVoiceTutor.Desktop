...
10. Keep backend current-state docs aligned with current production: the live symlink is `0.1.35-backend.140`, rollback is `0.1.35-backend.139`; verify `/health` and `/api/health/database` before any separate backend operation. The 2026-07-11 `.112`/`.111` verification remains historical only.
