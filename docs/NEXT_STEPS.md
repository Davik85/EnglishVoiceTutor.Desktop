...
Authenticated RTDN receipt, protected token persistence, linked-purchase-token replacement handling, a disabled-by-default reconciliation worker, and protected pending-refund review intake/processing are implemented locally. Pending-refund review is disabled by default, persists no raw pending token or order ID, uses only explicitly configured `NEUTRAL`, submits no fabricated usage evidence, and never changes Premium directly. Migration `20260803052655_AddGooglePlayPendingRefundReviewFoundation` exists locally and has not been applied. Migration `20260802154345_AddGooglePlayRtdnPersistenceFoundation` remains unapplied. SQL inspection, migration application, and deployment are manual owner steps following `COMMAND_PLAYBOOK.md`. Production remains `0.1.35-backend.138`; Google Play, RTDN, reconciliation, and pending-refund review remain disabled. The next major stage, after local acceptance, is one controlled server preparation, migration, and disabled backend deployment sequence.

The deployed disabled Google Play purchase-verification foundation remains in production `0.1.35-backend.138` with `.137` retained as rollback, and migration `20260727045935_AddGooglePlayPurchaseClaims` remains applied. The newer local RTDN receipt, protected-token persistence, reconciliation, linked-purchase-token replacement, and pending-refund review foundations remain disabled in production; migration `20260802154345_AddGooglePlayRtdnPersistenceFoundation` remains unapplied, and migration `20260803052655_AddGooglePlayPendingRefundReviewFoundation` exists locally and still requires manual SQL inspection and application.

That later work must preserve valid trial, `manual_admin`, and Paddle entitlement plus the final Desktop/Mobile Premium expiry; a Google Play failure must never revoke, shorten, hide, replace, or relabel that Premium. Provider-specific metadata selection must remain separate from provider-neutral Premium coverage calculation. The [mandatory billing-adapter regression gate](subscription-billing-foundation.md#mandatory-billing-adapter-regression-gate) applies before any Google Play work is accepted. Google Play, RTDN, reconciliation, and pending-refund review remain disabled in production; pending-refund messages return 503 until the module is explicitly enabled after the manual migration/deployment sequence. Test purchases remain unsupported; Paddle production behavior remains unchanged.
...
- Backend: production healthy at `https://api.languagevoicetutor.com` on `0.1.35-backend.138`, with `.137` as rollback; the trial/manual Premium expiry correction is production-verified without a migration or client release. CMS published version `51` remains the valid `CmsPublishedSnapshot` runtime source with `fallbackUsed=false`, 26 scenarios, 130 localized setup-message templates, and 625 localized context titles. The execution migration, disabled Google Play claim-table migration, and complete account-deletion workflow are production-deployed and production-verified; CMS/Admin login security and persistent role management are production-verified.
...
10. Keep backend current-state docs aligned with current production: the live symlink is `0.1.35-backend.138`, rollback is `0.1.35-backend.137`; verify `/health` and `/api/health/database` before any separate backend operation. The 2026-07-11 `.112`/`.111` verification remains historical only.
