...
The deployed disabled Google Play purchase-verification foundation and the newer RTDN/reconciliation/linked-purchase/pending-refund foundations remain present in production `.141`; `.140` is retained as rollback. Migration `20260727045935_AddGooglePlayPurchaseClaims` and both newer Google Play migrations are applied. The next stage is a separate reviewed production-enablement preparation phase for Backend Data Protection key-ring/certificate persistence, Android Publisher authentication, Google Cloud RTDN and Play Console configuration, and controlled validation. Source support for active-plus-previous Data Protection certificate decryption exists, but no certificate or key ring is provisioned and Data Protection is not enabled. None of the enablement prerequisites are complete, and all modules remain disabled until separately approved.

Repository source additionally contains migration `20260827105749_AddGooglePlayTrialDeferralFoundation` and the exactly-once initial account-wide Premium deferral workflow for Product ID `premium`; that migration is not applied in production. A later separately approved rollout must review and apply that additive migration before enabling the workflow. Exact Base Plan ID `monthly` has been saved in Play Console as a draft but is not activated. The launch base plan has no Google Play free-trial or introductory offer: the seven-day trial remains backend-owned, contiguous already-owned manual/provider Premium is preserved, and positive sub-day required defer uses the documented 24-hour minimum for `purchases.subscriptionsv2.defer`. Provider expiry remains authoritative after a fresh GET; no local target expiry may be synthesized from the stored duration.
...
Repository source now scopes Paddle activation, lifecycle expiry, full-refund, and chargeback mutation to the exact Paddle-owned `Subscription`/linked entitlement. Ambiguous legacy `provider_event` rows with `SubscriptionId == null` are never retroactively claimed by Paddle and fail closed for manual review; Google, `manual_admin`, trial, and other Paddle subscription coverage are not Paddle mutation targets. This repository-only correction is not a production enablement or deployment claim.

Repository source now includes bounded provider-renewal purchase gating. The additive authenticated status fields are calculated from all relevant Premium provider subscriptions rather than the single provider snapshot selected for display. Active/trialing Paddle is confirmed non-renewing only when the subscription's latest provider event matches a processed Paddle `BillingEvent` whose safe metadata proves the same user/subscription/event identity, a complete scheduled-change snapshot, `cancel`, and a strictly future effective time. Legacy or otherwise unproven sticky cancellation fields fail closed, as does a past effective time while local state remains active/trialing. Paddle lifecycle persistence treats a complete `subscription.updated` snapshot with explicit `scheduled_change: null` as authoritative removal of an earlier cancel/pause/resume; missing or invalid scheduled-change evidence never implies removal. Active renewal after removal and every recoverable payment-retry/past-due state remain blocking even when cancellation metadata is also present; paused/resumable state blocks, and multiple, incomplete, or conflicting ownership fails closed. Mobile parses the gate fail-closed, suppresses the new-purchase action unless the backend explicitly allows it, and fetches a fresh authenticated status for the same authenticated user immediately before store launch. Restore and already-owned token reverification remain independent. Remaining pre-enablement work is configuration, migration review/application, draft Base Plan activation, controlled sandbox validation, and separately reviewed production runtime enablement; no provider is automatically canceled, converted, or switched, and production Google Play remains disabled.
...
Windows Direct `1.6` and backend `.141` have already been published and verified. Do not list the legacy product-limit correction, `.141` deployment, or 1.6 upload as pending, and do not re-upload or repackage 1.6 unless a new release is intentionally prepared. Direct installer code signing, SmartScreen/trust mitigation, monitoring, customer-feedback triage, legal/support/operational review, and broader readiness remain pending. Google Play enablement remains separate and disabled.
...
10. Keep backend current-state docs aligned with current production: the live symlink is `0.1.35-backend.141`, rollback is `0.1.35-backend.140`; verify `/health` and `/api/health/database` before any separate backend operation. The 2026-07-11 `.112`/`.111` verification remains historical only.
