...
Authenticated RTDN receipt, protected token persistence, linked-purchase-token replacement handling, and a disabled-by-default local reconciliation worker are implemented. Immediate and bounded deferred replacements are supported; unsupported multi-item/add-on shapes fail closed, and old linked claims remain ownership records while old secrets are superseded. Pending-refund review notifications remain deliberately retryable with HTTP 503 until an encrypted pending-refund workflow exists. Migration `20260802154345_AddGooglePlayRtdnPersistenceFoundation` is not applied to production. Production remains `0.1.35-backend.138`; no Google Cloud, Play Console, migration application, or deployment has occurred. Protected pending-refund processing remains a separate local prerequisite before production RTDN enablement.

The deployed disabled Google Play purchase-verification foundation remains in production `0.1.35-backend.138` with `.137` retained as rollback, and migration `20260727045935_AddGooglePlayPurchaseClaims` remains applied. The newer local RTDN receipt, protected-token persistence, reconciliation, and linked-purchase-token replacement foundations remain disabled in production; their migration `20260802154345_AddGooglePlayRtdnPersistenceFoundation` is not applied. Protected pending-refund processing remains the separate local prerequisite before any production RTDN enablement, Google Console, sandbox, server-preparation, migration, or deployment work.
