...
Before Google Play Mobile purchase UI is enabled, add bounded provider-renewal purchase gating. The current provider-neutral status contract preserves the correct final Premium expiry but exposes only the effective linked provider snapshot, so Mobile cannot reliably distinguish every active or scheduled external auto-renew owner, especially Paddle, from all account states. Do not solve this remaining pre-enablement blocker by canceling, converting, or switching a provider automatically; define and approve the additive backend signal and Mobile suppression behavior separately.
