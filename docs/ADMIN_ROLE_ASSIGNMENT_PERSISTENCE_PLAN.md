...
## 2026-06-21 Admin UI Role Management MVP update

A minimal Admin UI Role Management MVP now exists under the guarded Admin UI. It uses only the existing `AdminRoleManagementPermissionPolicyName`-protected backend role-assignment endpoints for actor mapping, diagnostics, provisioning an `AdminUser` mapping for an existing app user, assigning roles, revoking roles, disabling admins, and enabling admins.
