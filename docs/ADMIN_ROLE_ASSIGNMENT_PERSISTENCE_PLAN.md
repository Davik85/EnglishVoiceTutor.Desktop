...
A read-only persistent role assignment service (`IAdminRoleAssignmentReadService` / `AdminRoleAssignmentReadService`) now exists to load active, non-revoked roles for active, non-disabled persistent admin users from the new EF tables. A trusted actor resolver (`IAdminRoleAssignmentActorResolver` / `AdminRoleAssignmentActorResolver`) now exists for role assignment workflows; it derives actor mapping from server-side authenticated claims and read-only persistent role-assignment reads, and it does not trust actor admin ids or actor role ids from request bodies. A read-only current-actor mapping diagnostics endpoint (`GET /api/admin/role-assignments/actor`) now uses that trusted resolver to report whether the authenticated admin maps to a persistent `AdminUser` and actor role ids; it does not expose email, raw claims, tokens, raw metadata, or provider payloads, does not mutate role assignment state, and does not activate persistent roles globally. A validation-only role assignment safety service (`IAdminRoleAssignmentSafetyService` / `AdminRoleAssignmentSafetyService`) exists for assign, revoke, and disable validation; it performs conservative read-only checks, does not mutate role assignments, does not create audit events, and is not wired into authorization. An audit-event writer service (`IAdminRoleAssignmentAuditService` / `AdminRoleAssignmentAuditService`) exists as an isolated operation seam; it appends only to `admin_role_assignment_events` and does not assign or revoke roles. The guarded assign and revoke endpoints still fail closed when actor mapping is unavailable. A guarded backend-only disable-admin endpoint (`POST /api/admin/role-assignments/disable-admin`) now exists. It disables only an existing persistent `AdminUser`, uses the trusted actor resolver and internal write service, does not trust actor admin id or actor role ids from the request body, does not create `AdminUser` rows or invites, does not revoke roles automatically, does not independently add or require Admin UI role management, and does not activate persistent roles globally. A guarded backend-only provision-admin-user endpoint (`POST /api/admin/role-assignments/provision-admin-user`) now exists. It provisions only a persistent `AdminUser` mapping for an existing app user, uses the trusted actor resolver and internal provisioning service, does not trust actor id or actor roles from the request body, does not accept email-based provisioning, does not assign roles, does not create app users or invites, does not independently add or require Admin UI role management, and does not activate persistent roles globally. `AdminPermissionAuthorizationHandler` now evaluates persistent active Admin role assignments for `AdminPermission:*` policies by resolving the authenticated principal through the read-only role assignment service and checking active role ids against the static role-to-permission catalog. BootstrapAdmin fallback is still preserved during controlled tester rollout. Persistent roles are active only for `AdminPermission:*` policies and do not authorize unmigrated `BootstrapAdmin`-only endpoints. Eligible existing read-only Admin endpoints, the CMS authoring workflow endpoints for draft save, validation, preview summary, publish, and version restore, and the user-impacting Admin action endpoints for Premium grant/revoke, free lesson allowance reset, and billing cancel-renewal have now been batch-migrated to narrow `AdminPermission:*` policies, bringing the total to exactly 35 existing Admin endpoint registrations protected directly by AdminPermission policies. CMS import/init endpoints remain BootstrapAdmin-protected; create-admin/invite endpoints do not exist, and Admin UI role management now exists as a controlled-validation product but was not expanded by the CMS authoring workflow migration or the user-impacting Admin action endpoint migration. Production Admin RBAC remains incomplete until operational validation, remaining endpoint migration, BootstrapAdmin fallback is narrowed or removed by owner-approved cutover, and rollback procedures are finalized.

Production Admin RBAC is still incomplete. The current controlled-testing model keeps BootstrapAdmin fallback preserved, with exactly 35 existing Admin endpoint registrations now protected by narrow `AdminPermission:*` policies after the eligible read-only, CMS authoring workflow, and user-impacting Admin action endpoint batch migrations. Role-assignment management endpoints remain protected by `AdminRoleManagementPermissionPolicyName`. CMS import/init endpoints and other still-unmigrated mutating or sensitive endpoints remain protected by BootstrapAdmin; Premium grant/revoke, free-lesson reset, and billing cancel-renewal now use narrow AdminPermission policies with BootstrapAdmin fallback preserved. Admin UI role management now exists as a product with the operational runbook `docs/ADMIN_ROLE_MANAGEMENT_UI_RUNBOOK.md`. The release gate includes `tools/test_admin_ui_role_management_policy.py`, and the opt-in manual smoke script `tools/smoke_admin_role_management_flow.ps1` exists for controlled validation. `bootstrap-first-owner` remains separate/manual and is not exposed in Admin UI. Production Admin RBAC remains incomplete until final operational validation, owner-approved BootstrapAdmin fallback narrowing/removal, and rollback procedures are finalized. The Admin RBAC cutover runbook (`docs/ADMIN_RBAC_CUTOVER_RUNBOOK.md`) and manual opt-in cutover validation smoke (`tools/smoke_admin_rbac_cutover_validation.ps1`) document the controlled fallback ON/OFF validation path; fallback remains enabled by default, and disabling it requires owner-approved controlled validation with rollback readiness. This plan describes the future persistence, safety, audit, rollout, validation, and rollback requirements before production role assignment can be enabled.
...
Internal enable-admin write support now exists in the backend role-assignment write/safety layer only. It is not exposed through HTTP yet, can only reactivate an existing disabled persistent `AdminUser` after Owner/SuperAdmin actor validation, and does not assign roles, create `AdminUser` rows, create invites, add Admin UI role management, or activate persistent roles globally. `AdminPermissionAuthorizationHandler` still does not use persistent roles or role-assignment read/safety/audit/write/actor/bootstrap/provisioning/enable services. Production Admin RBAC remains incomplete until a guarded enable-admin endpoint, invite workflow, operational UI, persistent role evaluation, endpoint-level enforcement, and rollback procedures are completed.
...
## 2026-06-21 backend-only enable-admin endpoint update

A guarded backend-only enable-admin endpoint now exists at `POST /api/admin/role-assignments/enable-admin`. It reactivates only an existing disabled persistent `AdminUser`, is protected by `AdminRoleManagementPermissionPolicyName`, accepts only `targetAdminUserId`, `reason`, and optional `safeMetadataJson`, derives the actor AdminUser id and actor role ids through `IAdminRoleAssignmentActorResolver`, and does not trust actor id or actor role values from the request body.

The endpoint delegates enablement to `IAdminRoleAssignmentWriteService.EnableAdminAsync` and does not accept email-based enable, assign roles, mutate EF entities, call `SaveChanges`, create application users, create `AdminUser` rows, create invites, add Admin UI role management, or write audit events directly. Persistent roles are still not activated globally: `AdminPermissionAuthorizationHandler` still does not evaluate persistent role-assignment tables or read/safety/audit/write/actor/bootstrap/provisioning/enable services. Production Admin RBAC remains incomplete until invite workflow, operational UI, persistent role evaluation, endpoint-level enforcement, and rollback procedures are completed.

## 2026-06-21 Admin UI Role Management controlled-validation surface update

A minimal Admin UI Role Management controlled-validation surface now exists under the guarded Admin UI. It uses only the existing `AdminRoleManagementPermissionPolicyName`-protected backend role-assignment endpoints for actor mapping, diagnostics, provisioning an `AdminUser` mapping for an existing app user, assigning roles, revoking roles, disabling admins, and enabling admins.

The UI does not expose `bootstrap-first-owner`, does not create application users, does not create invites, does not accept email/password/token/invite fields for provisioning, and does not remove BootstrapAdmin fallback. It is an operational controlled-rollout tool only and does not complete production Admin RBAC. Production Admin RBAC remains incomplete until full operational validation, remaining endpoint migration, owner-approved BootstrapAdmin fallback narrowing/removal, and rollback procedures are finalized.
