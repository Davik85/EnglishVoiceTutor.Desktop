...
Current release-readiness status: the public direct Windows manifest must be verified from live `latest.json`; the current verified public direct build is `1.1` on channel `direct-public` with installer `LanguageVoiceTutorSetup-1.1.exe`. Clean-machine smoke, small-screen/tablet visual smoke, the localized Russian/French Welcome fix, the admin roles/permissions policy and UI policy tests, the desktop release gate, and backend `0.1.35-backend.108` health checks, Admin statistics device metric wording/boundary verification, Admin successful payment statistics verification, and Admin RBAC / persistent role verification have passed. Code signing / SmartScreen mitigation, website/legal/support/ops final review, broader paid-launch approval, critical-change approval, customer portal work, and broader public-readiness work remain follow-up items. See `docs/CURRENT_STATE.md`, `docs/NEXT_STEPS.md`, and `docs/TESTER_RELEASE.md`.
