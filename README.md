...
Current release-readiness status: the private tester/direct Windows manifest must be verified from live `latest.json`; the last verified public snapshot was `0.1.35-tester.1`, while local `0.1.36-tester.2` artifacts are not public/live unless the website manifest points to them. Clean-machine smoke, update-over-existing-install validation, small-group external tester handoff, feedback collection, code signing, production billing, CMS published-snapshot learner runtime publishing, and broader public-readiness work remain follow-up items. See `docs/CURRENT_STATE.md`, `docs/NEXT_STEPS.md`, and `docs/TESTER_RELEASE.md`.
...
Recommended next work: clean-machine smoke, update-over-existing-install validation, small controlled external tester handoff, and feedback collection. CMS/Admin is connected and the `static-json-v1` content pack is initialized as Draft/admin content. Learner runtime remains static JSON by default; CMS published-snapshot runtime reads remain config-gated and must not be described as production-live for learners unless explicitly enabled and validated later. The deployed backend API is available at `https://api.languagevoicetutor.com`. Production billing readiness, remaining billing operations, production RBAC, critical-change approval workflow, Microsoft Store, App Store, Google Play, Mac version, broad public release readiness, and full production CMS/Admin operational readiness remain deferred. Prompt/scenario/bot-behavior polishing stays in CMS/Admin rather than code edits.
