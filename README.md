...
Recommended next work: continue the CMS/Admin content MVP before external tester handoff. Step 5D-6e is complete: the Scenarios editor now has compact local **Jump to** navigation, collapsible and visually separated structured sections for Basic fields, Lesson setup, Context selection / choices, Conversation flow / response guidance, Wrap-up / summary guidance, and Advanced JSON, plus helper text for normal content editors. Structured fields remain the recommended normal editing path; Advanced JSON remains a visually separated technical fallback for rare full-JSON edits. `Save draft` is still explicit and draft-only, does not publish runtime content, and continues to guide admins to publish after saving. Publishing still happens only from **Versions & Publish** through the confirmed publish flow and requires a publish summary for changed content; old published versions remain immutable, and restore creates a new published version instead of mutating history. Runtime learner behavior is unchanged by default: CMS published-snapshot runtime reads remain config-gated and static JSON fallback remains available. The next recommended implementation step is another CMS/admin improvement, not billing: refine validation/preview and content QA workflow ergonomics for practical admin content review. Production billing readiness, remaining billing operations, production RBAC, critical-change approval workflow, external tester handoff, public release readiness, and full production CMS/Admin operational readiness remain deferred. Prompt/scenario/bot-behavior polishing stays in CMS/Admin rather than code edits.
