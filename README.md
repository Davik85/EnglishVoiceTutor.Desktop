...
The Settings screen footer displays the installed app version, for example `Version: v1.4`. Users should include that version when reporting bugs.
...
Current release-readiness status: the public direct Windows manifest must be verified from live `latest.json`; the current verified public direct build is `1.4` on channel `direct-public` with installer `LanguageVoiceTutorSetup-1.4.exe` and production backend `0.1.35-backend.140`. Windows 1.4 adds the ORRALEN application-icon and version-specific shortcut-icon branding slice while preserving the Language Voice Tutor product name, stable AppId, accounts, and update continuity; broader Desktop and Mobile product-facing rebranding remains future audited work. Code signing / SmartScreen mitigation, website/legal/support/ops final review, monitoring, customer feedback, and broader public-readiness work remain follow-up items. See `docs/CURRENT_STATE.md`, `docs/NEXT_STEPS.md`, and `docs/TESTER_RELEASE.md`.
...
Step 5B-4 added a desktop release smoke gate in `docs/desktop-release-smoke-gate.md` and the safe local helper `tools/run_desktop_release_gate.ps1`. The reusable Windows direct-download installer flow is Inno Setup: set `$ReleaseVersion = "<release-version>"`, then run `powershell -ExecutionPolicy Bypass -File .\scripts\package-windows-inno-release.ps1 -Version $ReleaseVersion`; local artifacts are not public/live until upload and HTTPS manifest verification. Windows Direct Release 1.4 is public: public download, in-app manual-confirmation update, installed application launch, and public installer SHA-256 were verified. The app does not silently auto-update; no backend deployment or migration was part of that Windows release. Current production backend is `0.1.35-backend.140` with `.139` rollback. Code signing / SmartScreen mitigation, monitoring, customer feedback, and broader production-readiness follow-ups remain.
