...
Current release-readiness status: the private tester/direct Windows manifest must be verified from live `latest.json`; the current verified uploaded tester build is `0.1.36-tester.31` (previous tester release: `0.1.36-tester.30`). Clean-machine smoke, small-screen/tablet visual smoke, the localized Russian/French Welcome fix, the admin roles/permissions policy and UI policy tests, the desktop release gate, and backend `0.1.35-backend.77` health checks, Admin statistics device metric wording/boundary verification, Admin successful payment statistics verification, and Admin RBAC persistence migration verification have passed. Small-group external tester feedback collection, code signing, production billing, critical-change approval, and broader public-readiness work remain follow-up items. See `docs/CURRENT_STATE.md`, `docs/NEXT_STEPS.md`, and `docs/TESTER_RELEASE.md`.
