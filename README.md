...
Current release-readiness status: the public direct Windows manifest must be verified from live `latest.json`; the current verified public direct build is `1.2` on channel `direct-public` with installer `LanguageVoiceTutorSetup-1.2.exe` and production backend `0.1.35-backend.134`. Code signing / SmartScreen mitigation, website/legal/support/ops final review, monitoring, customer feedback, and broader public-readiness work remain follow-up items. See `docs/CURRENT_STATE.md`, `docs/NEXT_STEPS.md`, and `docs/TESTER_RELEASE.md`.
...
Step 5B-4 added a desktop release smoke gate in `docs/desktop-release-smoke-gate.md` and the safe local helper `tools/run_desktop_release_gate.ps1`. The reusable Windows direct-download installer flow is Inno Setup: set `$ReleaseVersion = "<release-version>"`, then run `powershell -ExecutionPolicy Bypass -File .\scripts\package-windows-inno-release.ps1 -Version $ReleaseVersion`; local artifacts are not public/live until upload and HTTPS manifest verification. Windows Direct Release 1.2 is public: the release gate, packaging, validation, upload, website download, 1.1-to-1.2 update, application launch, lesson smoke, and manual-confirmation update flow were verified. The app does not silently auto-update; no backend deployment or migration was part of this Windows release. Production backend remains `0.1.35-backend.134` with `.133` rollback. Code signing / SmartScreen mitigation, monitoring, customer feedback, and broader production-readiness follow-ups remain.
