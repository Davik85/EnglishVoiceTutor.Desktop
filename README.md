...
Current release-readiness status: the public direct Windows manifest must be verified from live `latest.json`; the current verified public direct build is `1.3` on channel `direct-public` with installer `LanguageVoiceTutorSetup-1.3.exe` and production backend `0.1.35-backend.140`. The public website presents ORRALEN as the operating/company/master brand and ORRALEN TECHNOLOGIES LTD as the legal company identity, while Language Voice Tutor remains the current product/application name. The Desktop and Mobile clients have not yet completed a product-facing ORRALEN rebrand. Code signing / SmartScreen mitigation, website/legal/support/ops final review, monitoring, customer feedback, and broader public-readiness work remain follow-up items. See `docs/CURRENT_STATE.md`, `docs/NEXT_STEPS.md`, and `docs/TESTER_RELEASE.md`.
...
Step 5B-4 added a desktop release smoke gate in `docs/desktop-release-smoke-gate.md` and the safe local helper `tools/run_desktop_release_gate.ps1`. The reusable Windows direct-download installer flow is Inno Setup: set `$ReleaseVersion = "<release-version>"`, then run `powershell -ExecutionPolicy Bypass -File .\scripts\package-windows-inno-release.ps1 -Version $ReleaseVersion`; local artifacts are not public/live until upload and HTTPS manifest verification. Windows Direct Release 1.3 is public: public download, in-app manual-confirmation update, installed application launch, and localized lesson setup were verified. The app does not silently auto-update; no backend deployment or migration was part of that Windows release. Current production backend is `0.1.35-backend.140` with `.139` rollback. Code signing / SmartScreen mitigation, monitoring, customer feedback, and broader production-readiness follow-ups remain.
