...
A safe Ubuntu 24.04 backend deployment foundation is documented in [`docs/BACKEND_SERVER_DEPLOYMENT.md`](docs/BACKEND_SERVER_DEPLOYMENT.md). It packages the backend as a self-contained `linux-x64` archive from the local Windows development machine, uploads it to a versioned server release folder under `/opt/languagevoicetutor/backend`, and documents a systemd service plus nginx reverse proxy for `api.languagevoicetutor.com`. Last known production backend snapshot for this handoff state: `/opt/languagevoicetutor/backend/releases/0.1.35-backend.50` is active via `/opt/languagevoicetutor/backend/current`, with `/opt/languagevoicetutor/backend/previous` pointing to `/opt/languagevoicetutor/backend/releases/0.1.35-backend.49`; verify the live value from the server symlink before calling it current. Backend `0.1.35-backend.50` is active/running, `/health` and `/api/health/database` return `200 OK`, includes the clarified Admin CMS Overview runtime-source labels, retains the Phase 5C Production logging hardening first deployed in `0.1.35-backend.40`, deploys the Admin statistics device metric wording/boundary fix plus successful payment statistics, and production migration `20260620165657_AddAdminRoleAssignmentPersistence` is applied for Admin RBAC persistence. The deployed backend API is available at `https://api.languagevoicetutor.com`, while broad public release remains deferred.
...
Current release-readiness status: the private tester/direct Windows manifest must be verified from live `latest.json`; the current verified uploaded tester build is `0.1.36-tester.24` (previous tester release: `0.1.36-tester.23`). Clean-machine smoke, small-screen/tablet visual smoke, the localized Russian/French Welcome fix, the admin roles/permissions policy and UI policy tests, the desktop release gate, and backend `0.1.35-backend.50` health checks, Admin statistics device metric wording/boundary verification, Admin successful payment statistics verification, and Admin RBAC persistence migration verification have passed. Small-group external tester feedback collection, code signing, production billing, critical-change approval, and broader public-readiness work remain follow-up items. See `docs/CURRENT_STATE.md`, `docs/NEXT_STEPS.md`, and `docs/TESTER_RELEASE.md`.
