...
public sealed class AiModelProviderAccessTestService
{
    ...
    public async Task<AiModelProviderTestResponse> TestDraftAsync(AiModelSettings draft, CancellationToken cancellationToken)
    {
        ...
        var overall = tested.Length > 0 && tested.All(result => result.ProviderOk == true) && results.All(result => result.SyntaxValid)
            ? "success"
            : tested.Any(result => result.ProviderOk == false) || results.Any(result => !result.SyntaxValid)
                ? "failed"
                : "partial";
        if (results.Any(result => !result.ProviderTested) && overall == "success")
        {
            overall = "partial";
        }

        return new AiModelProviderTestResponse(overall, results);
    }
    ...
    private async Task<AiModelProviderTestResult> TestTextRoleAsync(string roleId, string roleLabel, string modelId, AiModelSettingsValidationResponse validation, string apiKey, CancellationToken cancellationToken)
    {
        ...
        if (!syntaxValid)
        {
            return new(roleId, roleLabel, modelId, false, false, false, AiModelProviderTestCategories.InvalidRequest, "Model ID failed format validation; provider access was not tested.", null, null);
        }

        if (string.IsNullOrWhiteSpace(apiKey))
        {
            return new(roleId, roleLabel, modelId, true, false, null, AiModelProviderTestCategories.NotTested, "OpenAI API key is not configured on the server; provider access was not tested.", null, null);
        }
        ...
        try
        {
            ...
            {
                return new(roleId, roleLabel, modelId, true, true, true, AiModelProviderTestCategories.Ok, "Provider accepted a minimal safe Responses API request for this model.", (int)response.StatusCode, stopwatch.ElapsedMilliseconds);
            }

            var category = OpenAiProviderErrorMapper.MapStatusCode(response.StatusCode);
            return new(roleId, roleLabel, modelId, true, true, false, category, ToSafeMessage(category), (int)response.StatusCode, stopwatch.ElapsedMilliseconds);
        }
        catch (Exception ex) when (ex is OperationCanceledException or TimeoutException or HttpRequestException)
        {
            stopwatch.Stop();
            var category = OpenAiProviderErrorMapper.MapException(ex);
            return new(roleId, roleLabel, modelId, true, true, false, category, ToSafeMessage(category), null, stopwatch.ElapsedMilliseconds);
        }
    }
    ...
    private static bool IsRoleSyntaxValid(AiModelSettingsValidationResponse validation, string roleLabel) =>
        validation.Errors.All(error => !error.StartsWith(roleLabel, StringComparison.OrdinalIgnoreCase));
