...
builder.Services.Configure<GooglePlayRtdnOptions>(
    builder.Configuration.GetSection(GooglePlayRtdnOptions.SectionName));
...
builder.Services.Configure<RestoreCredentialsOptions>(builder.Configuration.GetSection(RestoreCredentialsOptions.SectionName));
var restoreCredentialsOptions = builder.Configuration.GetSection(RestoreCredentialsOptions.SectionName).Get<RestoreCredentialsOptions>() ?? new RestoreCredentialsOptions();
restoreCredentialsOptions.ValidateWhenEnabled();
builder.Services.AddScoped<IRestoreCredentialsService, RestoreCredentialsService>();
builder.Services.AddScoped<IRestoreCredentialsWebAuthnVerifier>(services => new RestoreCredentialsWebAuthnVerifier(services.GetRequiredService<Microsoft.Extensions.Options.IOptions<RestoreCredentialsOptions>>().Value));
...
builder.Services.AddSingleton<IGooglePlayPendingRefundFingerprintService, GooglePlayPendingRefundFingerprintService>();
builder.Services.AddScoped<GooglePlayRtdnEventPersistenceService>();
builder.Services.AddScoped<GooglePlayPendingRefundReviewPersistenceService>();
builder.Services.AddSingleton<IGoogleJsonWebSignatureValidator, GoogleJsonWebSignatureValidator>();
builder.Services.AddScoped<IGooglePlayPubSubOidcTokenValidator, GooglePlayPubSubOidcTokenValidator>();
...
builder.Services.AddGooglePlayPendingRefundReview(builder.Configuration);
...
builder.Services.AddScoped<ProviderSubscriptionPeriodPersistenceService>();
builder.Services.AddScoped<IProviderSubscriptionPeriodPersistenceService>(services => services.GetRequiredService<ProviderSubscriptionPeriodPersistenceService>());
builder.Services.AddScoped<IGooglePlayVerifiedPurchasePersistenceService, GooglePlayVerifiedPurchasePersistenceService>();
...
app.MapGooglePlayRtdnEndpoints();
...
static async Task<IResult> HandleGetProgressAsync(
    IProgressService progressService,
    ILoggerFactory loggerFactory,
    CancellationToken cancellationToken)
{
    var logger = loggerFactory.CreateLogger("ProgressEndpoint");

    try
    {
        return Results.Ok(await progressService.GetProgressAsync(cancellationToken));
    }
    catch (Exception exception) when (IsLessonSessionStorageUnavailable(exception))
    {
