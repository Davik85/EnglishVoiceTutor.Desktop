...
public static class WebsiteAdminEndpoints
{
    public static void MapWebsiteAdminEndpoints(this WebApplication app)
    {
        ...
        var get = app.MapGet(ApiConstants.AdminWebsiteHomeHeaderRoute, async (IWebsiteContentService service, CancellationToken cancellationToken) => Results.Ok(await service.GetAsync(cancellationToken)))
            .RequireAuthorization(AdminAuthorizationConstants.BootstrapAdminPolicyName);
        var save = app.MapPost(ApiConstants.AdminWebsiteHomeHeaderDraftRoute, async (WebsiteHomeHeaderContent request, IWebsiteContentService service, CancellationToken cancellationToken) => Results.Ok(await service.SaveDraftAsync(request, cancellationToken)))
            .RequireAuthorization(AdminAuthorizationConstants.BootstrapAdminPolicyName);
        var publish = app.MapPost(ApiConstants.AdminWebsiteHomeHeaderPublishRoute, async (WebsiteHomeHeaderContent request, IWebsiteContentService service, CancellationToken cancellationToken) =>
        {
